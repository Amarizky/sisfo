...
//
// Table class for master_kampus
//
class cmaster_kampus extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'master_kampus';
		$this->TableName = 'master_kampus';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`master_kampus`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// KampusID
		$this->KampusID = new cField('master_kampus', 'master_kampus', 'x_KampusID', 'KampusID', '`KampusID`', '`KampusID`', 200, -1, FALSE, '`KampusID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->KampusID->Sortable = TRUE; // Allow sort
		$this->fields['KampusID'] = &$this->KampusID;

		// Nama
		$this->Nama = new cField('master_kampus', 'master_kampus', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Nama->Sortable = TRUE; // Allow sort
		$this->fields['Nama'] = &$this->Nama;

		// Alamat
		$this->Alamat = new cField('master_kampus', 'master_kampus', 'x_Alamat', 'Alamat', '`Alamat`', '`Alamat`', 200, -1, FALSE, '`Alamat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Alamat->Sortable = TRUE; // Allow sort
		$this->fields['Alamat'] = &$this->Alamat;

		// ProvinsiID
		$this->ProvinsiID = new cField('master_kampus', 'master_kampus', 'x_ProvinsiID', 'ProvinsiID', '`ProvinsiID`', '`ProvinsiID`', 200, -1, FALSE, '`ProvinsiID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// KabupatenKotaID
		$this->KabupatenKotaID = new cField('master_kampus', 'master_kampus', 'x_KabupatenKotaID', 'KabupatenKotaID', '`KabupatenKotaID`', '`KabupatenKotaID`', 200, -1, FALSE, '`KabupatenKotaID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// KecamatanID
		$this->KecamatanID = new cField('master_kampus', 'master_kampus', 'x_KecamatanID', 'KecamatanID', '`KecamatanID`', '`KecamatanID`', 200, -1, FALSE, '`KecamatanID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// DesaID
		$this->DesaID = new cField('master_kampus', 'master_kampus', 'x_DesaID', 'DesaID', '`DesaID`', '`DesaID`', 200, -1, FALSE, '`DesaID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Email
		$this->_Email = new cField('master_kampus', 'master_kampus', 'x__Email', 'Email', '`Email`', '`Email`', 200, -1, FALSE, '`Email`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->_Email->Sortable = TRUE; // Allow sort
		$this->_Email->FldDefaultErrMsg = $Language->Phrase("IncorrectEmail");
		$this->fields['Email'] = &$this->_Email;

		// Telepon
		$this->Telepon = new cField('master_kampus', 'master_kampus', 'x_Telepon', 'Telepon', '`Telepon`', '`Telepon`', 200, -1, FALSE, '`Telepon`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Telepon->Sortable = TRUE; // Allow sort
		$this->fields['Telepon'] = &$this->Telepon;

		// Fax
		$this->Fax = new cField('master_kampus', 'master_kampus', 'x_Fax', 'Fax', '`Fax`', '`Fax`', 200, -1, FALSE, '`Fax`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Fax->Sortable = TRUE; // Allow sort
		$this->fields['Fax'] = &$this->Fax;

		// NA
		$this->NA = new cField('master_kampus', 'master_kampus', 'x_NA', 'NA', '`NA`', '`NA`', 202, -1, FALSE, '`NA`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`master_kampus`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('KampusID', $rs))
				ew_AddFilter($where, ew_QuotedName('KampusID', $this->DBID) . '=' . ew_QuotedValue($rs['KampusID'], $this->KampusID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`KampusID` = '@KampusID@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@KampusID@", ew_AdjustSql($this->KampusID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("master_kampusview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("master_kampusview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "master_kampusadd.php?" . $this->UrlParm($parm);
		else
			$url = "master_kampusadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("master_kampusedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("master_kampusadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("master_kampusdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "KampusID:" . ew_VarToJson($this->KampusID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->KampusID->CurrentValue)) {
			$sUrl .= "KampusID=" . urlencode($this->KampusID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["KampusID"]))
				$arKeys[] = ew_StripSlashes($_POST["KampusID"]);
			elseif (isset($_GET["KampusID"]))
				$arKeys[] = ew_StripSlashes($_GET["KampusID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->KampusID->CurrentValue = $key;
