...
$master_wilayah_kecamatan = NULL;

//
// Table class for master_wilayah_kecamatan
//
class cmaster_wilayah_kecamatan extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'master_wilayah_kecamatan';
		$this->TableName = 'master_wilayah_kecamatan';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`master_wilayah_kecamatan`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// KecamatanID
		$this->KecamatanID = new cField('master_wilayah_kecamatan', 'master_wilayah_kecamatan', 'x_KecamatanID', 'KecamatanID', '`KecamatanID`', '`KecamatanID`', 200, -1, FALSE, '`KecamatanID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->KecamatanID->Sortable = TRUE; // Allow sort
		$this->fields['KecamatanID'] = &$this->KecamatanID;

		// KabupatenKotaID
		$this->KabupatenKotaID = new cField('master_wilayah_kecamatan', 'master_wilayah_kecamatan', 'x_KabupatenKotaID', 'KabupatenKotaID', '`KabupatenKotaID`', '`KabupatenKotaID`', 200, -1, FALSE, '`KabupatenKotaID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Kecamatan
		$this->Kecamatan = new cField('master_wilayah_kecamatan', 'master_wilayah_kecamatan', 'x_Kecamatan', 'Kecamatan', '`Kecamatan`', '`Kecamatan`', 200, -1, FALSE, '`Kecamatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`master_wilayah_kecamatan`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('KecamatanID', $rs))
				ew_AddFilter($where, ew_QuotedName('KecamatanID', $this->DBID) . '=' . ew_QuotedValue($rs['KecamatanID'], $this->KecamatanID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`KecamatanID` = '@KecamatanID@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@KecamatanID@", ew_AdjustSql($this->KecamatanID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "master_wilayah_kecamatanlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("master_wilayah_kecamatanview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("master_wilayah_kecamatanview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "master_wilayah_kecamatanadd.php?" . $this->UrlParm($parm);
		else
			$url = "master_wilayah_kecamatanadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("master_wilayah_kecamatanedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("master_wilayah_kecamatanadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("master_wilayah_kecamatandelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "KecamatanID:" . ew_VarToJson($this->KecamatanID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->KecamatanID->CurrentValue)) {
			$sUrl .= "KecamatanID=" . urlencode($this->KecamatanID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["KecamatanID"]))
				$arKeys[] = ew_StripSlashes($_POST["KecamatanID"]);
			elseif (isset($_GET["KecamatanID"]))
				$arKeys[] = ew_StripSlashes($_GET["KecamatanID"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->KecamatanID->CurrentValue = $key;
   ...
   // Common render codes
	...
	function WriteAuditTrailDummy($typ) {
		$table = 'master_wilayah_kecamatan';
		$usr = CurrentUserName();
