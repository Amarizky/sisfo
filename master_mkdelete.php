...
$master_mk_delete = NULL; // Initialize page object first

class cmaster_mk_delete extends cmaster_mk {
	...
	// Page object name
	var $PageObjName = 'master_mk_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("master_mklist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in master_mk class, master_mkinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("master_mklist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($master_mk_delete)) $master_mk_delete = new cmaster_mk_delete();

// Page init
$master_mk_delete->Page_Init();

// Page main
$master_mk_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_mk_delete->Page_Render();
?>
...
var CurrentForm = fmaster_mkdelete = new ew_Form("fmaster_mkdelete", "delete");

// Form_CustomValidate event
fmaster_mkdelete.Form_CustomValidate = 
...
fmaster_mkdelete.ValidateRequired = true;
<?php } else { ?>
fmaster_mkdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmaster_mkdelete.Lists["x_Tingkat"] = {"LinkField":"x_Tingkat","Ajax":true,"AutoFill":false,"DisplayFields":["x_Tingkat","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"kelas"};
fmaster_mkdelete.Lists["x_Sesi"] = {"LinkField":"x_Sesi","Ajax":true,"AutoFill":false,"DisplayFields":["x_NamaSesi","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_sesi"};
fmaster_mkdelete.Lists["x_Wajib"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_mkdelete.Lists["x_Wajib"].Options = <?php echo json_encode($master_mk->Wajib->Options()) ?>;
fmaster_mkdelete.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_mkdelete.Lists["x_NA"].Options = <?php echo json_encode($master_mk->NA->Options()) ?>;
...
<?php $master_mk_delete->ShowPageHeader(); ?>
<?php
$master_mk_delete->ShowMessage();
?>
<form name="fmaster_mkdelete" id="fmaster_mkdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_mk_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_mk_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="master_mk">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($master_mk_delete->RecKeys as $key) { ?>
...
<?php if ($master_mk->MKKode->Visible) { // MKKode ?>
		<th><span id="elh_master_mk_MKKode" class="master_mk_MKKode"><?php echo $master_mk->MKKode->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_mk->Nama->Visible) { // Nama ?>
		<th><span id="elh_master_mk_Nama" class="master_mk_Nama"><?php echo $master_mk->Nama->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_mk->Singkatan->Visible) { // Singkatan ?>
		<th><span id="elh_master_mk_Singkatan" class="master_mk_Singkatan"><?php echo $master_mk->Singkatan->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_mk->Tingkat->Visible) { // Tingkat ?>
		<th><span id="elh_master_mk_Tingkat" class="master_mk_Tingkat"><?php echo $master_mk->Tingkat->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_mk->Sesi->Visible) { // Sesi ?>
		<th><span id="elh_master_mk_Sesi" class="master_mk_Sesi"><?php echo $master_mk->Sesi->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_mk->Wajib->Visible) { // Wajib ?>
		<th><span id="elh_master_mk_Wajib" class="master_mk_Wajib"><?php echo $master_mk->Wajib->FldCaption() ?></span></th>
<?php } ?>
<?php if ($master_mk->NA->Visible) { // NA ?>
		<th><span id="elh_master_mk_NA" class="master_mk_NA"><?php echo $master_mk->NA->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$master_mk_delete->RecCnt = 0;
$i = 0;
while (!$master_mk_delete->Recordset->EOF) {
	$master_mk_delete->RecCnt++;
	$master_mk_delete->RowCnt++;

	// Set row properties
	$master_mk->ResetAttrs();
	$master_mk->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$master_mk_delete->LoadRowValues($master_mk_delete->Recordset);

	// Render row
	$master_mk_delete->RenderRow();
?>
	<tr<?php echo $master_mk->RowAttributes() ?>>
<?php if ($master_mk->MKKode->Visible) { // MKKode ?>
		<td<?php echo $master_mk->MKKode->CellAttributes() ?>>
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_MKKode" class="master_mk_MKKode">
...
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_Nama" class="master_mk_Nama">
...
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_Singkatan" class="master_mk_Singkatan">
...
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_Tingkat" class="master_mk_Tingkat">
...
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_Sesi" class="master_mk_Sesi">
...
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_Wajib" class="master_mk_Wajib">
...
<span id="el<?php echo $master_mk_delete->RowCnt ?>_master_mk_NA" class="master_mk_NA">
...
<?php
	$master_mk_delete->Recordset->MoveNext();
}
$master_mk_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $master_mk_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmaster_mkdelete.Init();
</script>
<?php
$master_mk_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$master_mk_delete->Page_Terminate();
?>
