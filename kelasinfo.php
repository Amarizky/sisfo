...
//
// Table class for kelas
//
class ckelas extends cTable {
	...
	function __construct() {
		...
		// KelasID
		$this->KelasID = new cField('kelas', 'kelas', 'x_KelasID', 'KelasID', '`KelasID`', '`KelasID`', 200, -1, FALSE, '`KelasID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->KelasID->Sortable = TRUE; // Allow sort
		$this->fields['KelasID'] = &$this->KelasID;

		// Tingkat
		$this->Tingkat = new cField('kelas', 'kelas', 'x_Tingkat', 'Tingkat', '`Tingkat`', '`Tingkat`', 200, -1, FALSE, '`Tingkat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Tingkat->Sortable = TRUE; // Allow sort
		$this->fields['Tingkat'] = &$this->Tingkat;

		// Nama
		$this->Nama = new cField('kelas', 'kelas', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Nama->Sortable = TRUE; // Allow sort
		$this->fields['Nama'] = &$this->Nama;

		// ProdiID
		$this->ProdiID = new cField('kelas', 'kelas', 'x_ProdiID', 'ProdiID', '`ProdiID`', '`ProdiID`', 200, -1, FALSE, '`ProdiID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->ProdiID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ProdiID'] = &$this->ProdiID;

		// NA
		$this->NA = new cField('kelas', 'kelas', 'x_NA', 'NA', '`NA`', '`NA`', 202, -1, FALSE, '`NA`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`kelas`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('KelasID', $rs))
				ew_AddFilter($where, ew_QuotedName('KelasID', $this->DBID) . '=' . ew_QuotedValue($rs['KelasID'], $this->KelasID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@KelasID@", ew_AdjustSql($this->KelasID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("kelasview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("kelasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "kelasadd.php?" . $this->UrlParm($parm);
		else
			$url = "kelasadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("kelasedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("kelasadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("kelasdelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "KelasID:" . ew_VarToJson($this->KelasID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->KelasID->CurrentValue)) {
			$sUrl .= "KelasID=" . urlencode($this->KelasID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["KelasID"]))
				$arKeys[] = ew_StripSlashes($_POST["KelasID"]);
			elseif (isset($_GET["KelasID"]))
				$arKeys[] = ew_StripSlashes($_GET["KelasID"]);
			else
