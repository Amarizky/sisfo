...
$master_mk_list = NULL; // Initialize page object first

class cmaster_mk_list extends cmaster_mk {
	...
	// Page object name
	var $PageObjName = 'master_mk_list';

	// Grid form hidden field names
	var $FormName = 'fmaster_mklist';
	...
	function __construct() {
		...
		$this->AddUrl = "master_mkadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "master_mkdelete.php";
		$this->MultiUpdateUrl = "master_mkupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fmaster_mklistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fmaster_mklistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fmaster_mklistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fmaster_mklistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fmaster_mklistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fmaster_mklist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmaster_mklistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
		...
		if (ew_IsMobile())
			$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"master_mksrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		else
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-table=\"master_mk\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" onclick=\"ew_ModalDialogShow({lnk:this,url:'master_mksrch.php',caption:'" . $Language->Phrase("Search") . "'});\">" . $Language->Phrase("AdvancedSearchBtn") . "</button>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fmaster_mklistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_master_mk\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_master_mk',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fmaster_mklist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($master_mk_list)) $master_mk_list = new cmaster_mk_list();

// Page init
$master_mk_list->Page_Init();

// Page main
$master_mk_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_mk_list->Page_Render();
?>
...
var CurrentForm = fmaster_mklist = new ew_Form("fmaster_mklist", "list");
fmaster_mklist.FormKeyCountName = '<?php echo $master_mk_list->FormKeyCountName ?>';

// Validate form
fmaster_mklist.Validate = function() {
...
// Check empty row
fmaster_mklist.EmptyRow = function(infix) {
	var fobj = this.Form;
...
fmaster_mklist.Form_CustomValidate = 
...
fmaster_mklist.ValidateRequired = true;
<?php } else { ?>
fmaster_mklist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmaster_mklist.Lists["x_Tingkat"] = {"LinkField":"x_Tingkat","Ajax":true,"AutoFill":false,"DisplayFields":["x_Tingkat","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"kelas"};
fmaster_mklist.Lists["x_Sesi"] = {"LinkField":"x_Sesi","Ajax":true,"AutoFill":false,"DisplayFields":["x_NamaSesi","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_sesi"};
fmaster_mklist.Lists["x_Wajib"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_mklist.Lists["x_Wajib"].Options = <?php echo json_encode($master_mk->Wajib->Options()) ?>;
fmaster_mklist.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_mklist.Lists["x_NA"].Options = <?php echo json_encode($master_mk->NA->Options()) ?>;

// Form object for search
var CurrentSearchForm = fmaster_mklistsrch = new ew_Form("fmaster_mklistsrch");
</script>
<style type="text/css">
...
<?php } ?>
<?php if ($master_mk_list->TotalRecs > 0 && $master_mk_list->ExportOptions->Visible()) { ?>
<?php $master_mk_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($master_mk_list->SearchOptions->Visible()) { ?>
<?php $master_mk_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($master_mk_list->FilterOptions->Visible()) { ?>
<?php $master_mk_list->FilterOptions->Render("body") ?>
<?php } ?>
<?php if ($master_mk->Export == "") { ?>
...
<?php } ?>
<?php if (($master_mk->Export == "") || (EW_EXPORT_MASTER_RECORD && $master_mk->Export == "print")) { ?>
<?php
if ($master_mk_list->DbMasterFilter <> "" && $master_mk->getCurrentMasterTable() == "kurikulum") {
	if ($master_mk_list->MasterRecordExists) {
?>
...
if ($master_mk->CurrentAction == "gridadd") {
	...
	$master_mk_list->StartRec = 1;
	$master_mk_list->DisplayRecs = $master_mk->GridAddRowCount;
	$master_mk_list->TotalRecs = $master_mk_list->DisplayRecs;
	$master_mk_list->StopRec = $master_mk_list->DisplayRecs;
} else {
	$bSelectLimit = $master_mk_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($master_mk_list->TotalRecs <= 0)
			$master_mk_list->TotalRecs = $master_mk->SelectRecordCount();
	} else {
		if (!$master_mk_list->Recordset && ($master_mk_list->Recordset = $master_mk_list->LoadRecordset()))
			$master_mk_list->TotalRecs = $master_mk_list->Recordset->RecordCount();
	}
	$master_mk_list->StartRec = 1;
	if ($master_mk_list->DisplayRecs <= 0 || ($master_mk->Export <> "" && $master_mk->ExportAll)) // Display all records
		$master_mk_list->DisplayRecs = $master_mk_list->TotalRecs;
	if (!($master_mk->Export <> "" && $master_mk->ExportAll))
		$master_mk_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$master_mk_list->Recordset = $master_mk_list->LoadRecordset($master_mk_list->StartRec-1, $master_mk_list->DisplayRecs);

	// Set no record found message
	if ($master_mk->CurrentAction == "" && $master_mk_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$master_mk_list->setWarningMessage(ew_DeniedMsg());
		if ($master_mk_list->SearchWhere == "0=101")
			$master_mk_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$master_mk_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($master_mk_list->AuditTrailOnSearch && $master_mk_list->Command == "search" && !$master_mk_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $master_mk_list->getSessionWhere();
		$master_mk_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
}
$master_mk_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($master_mk->Export == "" && $master_mk->CurrentAction == "") { ?>
<form name="fmaster_mklistsrch" id="fmaster_mklistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($master_mk_list->SearchWhere <> "") ? " in" : ""; ?>
<div id="fmaster_mklistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="master_mk">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($master_mk_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($master_mk_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $master_mk_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($master_mk_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($master_mk_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($master_mk_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($master_mk_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $master_mk_list->ShowPageHeader(); ?>
<?php
$master_mk_list->ShowMessage();
?>
<?php if ($master_mk_list->TotalRecs > 0 || $master_mk->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid master_mk">
<?php if ($master_mk->Export == "") { ?>
<div class="panel-heading ewGridUpperPanel">
<?php if ($master_mk->CurrentAction <> "gridadd" && $master_mk->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($master_mk_list->Pager)) $master_mk_list->Pager = new cPrevNextPager($master_mk_list->StartRec, $master_mk_list->DisplayRecs, $master_mk_list->TotalRecs) ?>
<?php if ($master_mk_list->Pager->RecordCount > 0 && $master_mk_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($master_mk_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $master_mk_list->PageUrl() ?>start=<?php echo $master_mk_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($master_mk_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $master_mk_list->PageUrl() ?>start=<?php echo $master_mk_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $master_mk_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($master_mk_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $master_mk_list->PageUrl() ?>start=<?php echo $master_mk_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($master_mk_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $master_mk_list->PageUrl() ?>start=<?php echo $master_mk_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $master_mk_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $master_mk_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $master_mk_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $master_mk_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($master_mk_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fmaster_mklist" id="fmaster_mklist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_mk_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_mk_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="master_mk">
<?php if ($master_mk->getCurrentMasterTable() == "kurikulum" && $master_mk->CurrentAction <> "") { ?>
...
<?php if ($master_mk_list->TotalRecs > 0 || $master_mk->CurrentAction == "gridedit") { ?>
<table id="tbl_master_mklist" class="table ewTable">
...
// Header row
$master_mk_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$master_mk_list->RenderListOptions();

// Render list options (header, left)
$master_mk_list->ListOptions->Render("header", "left");
?>
<?php if ($master_mk->MKKode->Visible) { // MKKode ?>
	...
	<?php } else { ?>
		<th data-name="MKKode"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->MKKode) ?>',1);"><div id="elh_master_mk_MKKode" class="master_mk_MKKode">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $master_mk->MKKode->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($master_mk->MKKode->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($master_mk->MKKode->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($master_mk->Nama->Visible) { // Nama ?>
	...
	<?php } else { ?>
		<th data-name="Nama"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->Nama) ?>',1);"><div id="elh_master_mk_Nama" class="master_mk_Nama">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $master_mk->Nama->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($master_mk->Nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($master_mk->Nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($master_mk->Singkatan->Visible) { // Singkatan ?>
	...
	<?php } else { ?>
		<th data-name="Singkatan"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->Singkatan) ?>',1);"><div id="elh_master_mk_Singkatan" class="master_mk_Singkatan">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $master_mk->Singkatan->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($master_mk->Singkatan->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($master_mk->Singkatan->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($master_mk->Tingkat->Visible) { // Tingkat ?>
	...
	<?php } else { ?>
		<th data-name="Tingkat"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->Tingkat) ?>',1);"><div id="elh_master_mk_Tingkat" class="master_mk_Tingkat">
...
<?php if ($master_mk->Sesi->Visible) { // Sesi ?>
	...
	<?php } else { ?>
		<th data-name="Sesi"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->Sesi) ?>',1);"><div id="elh_master_mk_Sesi" class="master_mk_Sesi">
...
<?php if ($master_mk->Wajib->Visible) { // Wajib ?>
	...
	<?php } else { ?>
		<th data-name="Wajib"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->Wajib) ?>',1);"><div id="elh_master_mk_Wajib" class="master_mk_Wajib">
...
<?php if ($master_mk->NA->Visible) { // NA ?>
	...
	<?php } else { ?>
		<th data-name="NA"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $master_mk->SortUrl($master_mk->NA) ?>',1);"><div id="elh_master_mk_NA" class="master_mk_NA">
...
$master_mk_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($master_mk->ExportAll && $master_mk->Export <> "") {
	$master_mk_list->StopRec = $master_mk_list->TotalRecs;
} else {

	// Set the last record to display
	if ($master_mk_list->TotalRecs > $master_mk_list->StartRec + $master_mk_list->DisplayRecs - 1)
		$master_mk_list->StopRec = $master_mk_list->StartRec + $master_mk_list->DisplayRecs - 1;
	else
		$master_mk_list->StopRec = $master_mk_list->TotalRecs;
}
...
if ($objForm) {
	$objForm->Index = -1;
	if ($objForm->HasValue($master_mk_list->FormKeyCountName) && ($master_mk->CurrentAction == "gridadd" || $master_mk->CurrentAction == "gridedit" || $master_mk->CurrentAction == "F")) {
		$master_mk_list->KeyCount = $objForm->GetValue($master_mk_list->FormKeyCountName);
		$master_mk_list->StopRec = $master_mk_list->StartRec + $master_mk_list->KeyCount - 1;
	}
}
$master_mk_list->RecCnt = $master_mk_list->StartRec - 1;
if ($master_mk_list->Recordset && !$master_mk_list->Recordset->EOF) {
	$master_mk_list->Recordset->MoveFirst();
	$bSelectLimit = $master_mk_list->UseSelectLimit;
	if (!$bSelectLimit && $master_mk_list->StartRec > 1)
		$master_mk_list->Recordset->Move($master_mk_list->StartRec - 1);
} elseif (!$master_mk->AllowAddDeleteRow && $master_mk_list->StopRec == 0) {
	$master_mk_list->StopRec = $master_mk->GridAddRowCount;
}

// Initialize aggregate
$master_mk->RowType = EW_ROWTYPE_AGGREGATEINIT;
$master_mk->ResetAttrs();
$master_mk_list->RenderRow();
if ($master_mk->CurrentAction == "gridadd")
	$master_mk_list->RowIndex = 0;
if ($master_mk->CurrentAction == "gridedit")
	$master_mk_list->RowIndex = 0;
while ($master_mk_list->RecCnt < $master_mk_list->StopRec) {
	$master_mk_list->RecCnt++;
	if (intval($master_mk_list->RecCnt) >= intval($master_mk_list->StartRec)) {
		$master_mk_list->RowCnt++;
		if ($master_mk->CurrentAction == "gridadd" || $master_mk->CurrentAction == "gridedit" || $master_mk->CurrentAction == "F") {
			$master_mk_list->RowIndex++;
			$objForm->Index = $master_mk_list->RowIndex;
			if ($objForm->HasValue($master_mk_list->FormActionName))
				$master_mk_list->RowAction = strval($objForm->GetValue($master_mk_list->FormActionName));
			elseif ($master_mk->CurrentAction == "gridadd")
				$master_mk_list->RowAction = "insert";
			else
				$master_mk_list->RowAction = "";
		}

		// Set up key count
		$master_mk_list->KeyCount = $master_mk_list->RowIndex;

		// Init row class and style
		$master_mk->ResetAttrs();
		$master_mk->CssClass = "";
		if ($master_mk->CurrentAction == "gridadd") {
			$master_mk_list->LoadDefaultValues(); // Load default values
		} else {
			$master_mk_list->LoadRowValues($master_mk_list->Recordset); // Load row values
		}
		...
		if ($master_mk->CurrentAction == "gridadd" && $master_mk->EventCancelled && !$objForm->HasValue("k_blankrow")) // Insert failed
			$master_mk_list->RestoreCurrentRowFormValues($master_mk_list->RowIndex); // Restore form values
		if ($master_mk->CurrentAction == "gridedit") { // Grid edit
			if ($master_mk->EventCancelled) {
				$master_mk_list->RestoreCurrentRowFormValues($master_mk_list->RowIndex); // Restore form values
			}
			if ($master_mk_list->RowAction == "insert")
				$master_mk->RowType = EW_ROWTYPE_ADD; // Render add
			else
		...
		if ($master_mk->RowType == EW_ROWTYPE_EDIT) // Edit row
			$master_mk_list->EditRowCnt++;

		// Set up row id / data-rowindex
		$master_mk->RowAttrs = array_merge($master_mk->RowAttrs, array('data-rowindex'=>$master_mk_list->RowCnt, 'id'=>'r' . $master_mk_list->RowCnt . '_master_mk', 'data-rowtype'=>$master_mk->RowType));
		...
		if ($master_mk_list->RowAction <> "delete" && $master_mk_list->RowAction <> "insertdelete" && !($master_mk_list->RowAction == "insert" && $master_mk->CurrentAction == "F" && $master_mk_list->EmptyRow())) {
?>
	<tr<?php echo $master_mk->RowAttributes() ?>>
<?php

// Render list options (body, left)
$master_mk_list->ListOptions->Render("body", "left", $master_mk_list->RowCnt);
?>
	<?php if ($master_mk->MKKode->Visible) { // MKKode ?>
		<td data-name="MKKode"<?php echo $master_mk->MKKode->CellAttributes() ?>>
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_MKKode" class="form-group master_mk_MKKode">
<input type="text" data-table="master_mk" data-field="x_MKKode" name="x<?php echo $master_mk_list->RowIndex ?>_MKKode" id="x<?php echo $master_mk_list->RowIndex ?>_MKKode" size="7" maxlength="20" placeholder="<?php echo ew_HtmlEncode($master_mk->MKKode->getPlaceHolder()) ?>" value="<?php echo $master_mk->MKKode->EditValue ?>"<?php echo $master_mk->MKKode->EditAttributes() ?>>
</span>
<input type="hidden" data-table="master_mk" data-field="x_MKKode" name="o<?php echo $master_mk_list->RowIndex ?>_MKKode" id="o<?php echo $master_mk_list->RowIndex ?>_MKKode" value="<?php echo ew_HtmlEncode($master_mk->MKKode->OldValue) ?>">
<?php } ?>
...
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_MKKode" class="master_mk_MKKode">
<span<?php echo $master_mk->MKKode->ViewAttributes() ?>>
<?php echo $master_mk->MKKode->ListViewValue() ?></span>
</span>
<?php } ?>
<a id="<?php echo $master_mk_list->PageObjName . "_row_" . $master_mk_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<input type="hidden" data-table="master_mk" data-field="x_MKID" name="x<?php echo $master_mk_list->RowIndex ?>_MKID" id="x<?php echo $master_mk_list->RowIndex ?>_MKID" value="<?php echo ew_HtmlEncode($master_mk->MKID->CurrentValue) ?>">
<input type="hidden" data-table="master_mk" data-field="x_MKID" name="o<?php echo $master_mk_list->RowIndex ?>_MKID" id="o<?php echo $master_mk_list->RowIndex ?>_MKID" value="<?php echo ew_HtmlEncode($master_mk->MKID->OldValue) ?>">
<?php } ?>
...
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Nama" class="form-group master_mk_Nama">
<input type="text" data-table="master_mk" data-field="x_Nama" name="x<?php echo $master_mk_list->RowIndex ?>_Nama" id="x<?php echo $master_mk_list->RowIndex ?>_Nama" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($master_mk->Nama->getPlaceHolder()) ?>" value="<?php echo $master_mk->Nama->EditValue ?>"<?php echo $master_mk->Nama->EditAttributes() ?>>
</span>
<input type="hidden" data-table="master_mk" data-field="x_Nama" name="o<?php echo $master_mk_list->RowIndex ?>_Nama" id="o<?php echo $master_mk_list->RowIndex ?>_Nama" value="<?php echo ew_HtmlEncode($master_mk->Nama->OldValue) ?>">
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Nama" class="form-group master_mk_Nama">
<input type="text" data-table="master_mk" data-field="x_Nama" name="x<?php echo $master_mk_list->RowIndex ?>_Nama" id="x<?php echo $master_mk_list->RowIndex ?>_Nama" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($master_mk->Nama->getPlaceHolder()) ?>" value="<?php echo $master_mk->Nama->EditValue ?>"<?php echo $master_mk->Nama->EditAttributes() ?>>
...
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Nama" class="master_mk_Nama">
...
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Singkatan" class="form-group master_mk_Singkatan">
<input type="text" data-table="master_mk" data-field="x_Singkatan" name="x<?php echo $master_mk_list->RowIndex ?>_Singkatan" id="x<?php echo $master_mk_list->RowIndex ?>_Singkatan" size="7" maxlength="20" placeholder="<?php echo ew_HtmlEncode($master_mk->Singkatan->getPlaceHolder()) ?>" value="<?php echo $master_mk->Singkatan->EditValue ?>"<?php echo $master_mk->Singkatan->EditAttributes() ?>>
</span>
<input type="hidden" data-table="master_mk" data-field="x_Singkatan" name="o<?php echo $master_mk_list->RowIndex ?>_Singkatan" id="o<?php echo $master_mk_list->RowIndex ?>_Singkatan" value="<?php echo ew_HtmlEncode($master_mk->Singkatan->OldValue) ?>">
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Singkatan" class="form-group master_mk_Singkatan">
<input type="text" data-table="master_mk" data-field="x_Singkatan" name="x<?php echo $master_mk_list->RowIndex ?>_Singkatan" id="x<?php echo $master_mk_list->RowIndex ?>_Singkatan" size="7" maxlength="20" placeholder="<?php echo ew_HtmlEncode($master_mk->Singkatan->getPlaceHolder()) ?>" value="<?php echo $master_mk->Singkatan->EditValue ?>"<?php echo $master_mk->Singkatan->EditAttributes() ?>>
...
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Singkatan" class="master_mk_Singkatan">
...
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Tingkat" class="form-group master_mk_Tingkat">
<select data-table="master_mk" data-field="x_Tingkat" data-value-separator="<?php echo $master_mk->Tingkat->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $master_mk_list->RowIndex ?>_Tingkat" name="x<?php echo $master_mk_list->RowIndex ?>_Tingkat"<?php echo $master_mk->Tingkat->EditAttributes() ?>>
<?php echo $master_mk->Tingkat->SelectOptionListHtml("x<?php echo $master_mk_list->RowIndex ?>_Tingkat") ?>
</select>
<input type="hidden" name="s_x<?php echo $master_mk_list->RowIndex ?>_Tingkat" id="s_x<?php echo $master_mk_list->RowIndex ?>_Tingkat" value="<?php echo $master_mk->Tingkat->LookupFilterQuery() ?>">
</span>
<input type="hidden" data-table="master_mk" data-field="x_Tingkat" name="o<?php echo $master_mk_list->RowIndex ?>_Tingkat" id="o<?php echo $master_mk_list->RowIndex ?>_Tingkat" value="<?php echo ew_HtmlEncode($master_mk->Tingkat->OldValue) ?>">
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Tingkat" class="form-group master_mk_Tingkat">
<select data-table="master_mk" data-field="x_Tingkat" data-value-separator="<?php echo $master_mk->Tingkat->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $master_mk_list->RowIndex ?>_Tingkat" name="x<?php echo $master_mk_list->RowIndex ?>_Tingkat"<?php echo $master_mk->Tingkat->EditAttributes() ?>>
...
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Tingkat" class="master_mk_Tingkat">
...
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Sesi" class="form-group master_mk_Sesi">
<select data-table="master_mk" data-field="x_Sesi" data-value-separator="<?php echo $master_mk->Sesi->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $master_mk_list->RowIndex ?>_Sesi" name="x<?php echo $master_mk_list->RowIndex ?>_Sesi"<?php echo $master_mk->Sesi->EditAttributes() ?>>
<?php echo $master_mk->Sesi->SelectOptionListHtml("x<?php echo $master_mk_list->RowIndex ?>_Sesi") ?>
</select>
<input type="hidden" name="s_x<?php echo $master_mk_list->RowIndex ?>_Sesi" id="s_x<?php echo $master_mk_list->RowIndex ?>_Sesi" value="<?php echo $master_mk->Sesi->LookupFilterQuery() ?>">
</span>
<input type="hidden" data-table="master_mk" data-field="x_Sesi" name="o<?php echo $master_mk_list->RowIndex ?>_Sesi" id="o<?php echo $master_mk_list->RowIndex ?>_Sesi" value="<?php echo ew_HtmlEncode($master_mk->Sesi->OldValue) ?>">
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Sesi" class="form-group master_mk_Sesi">
<select data-table="master_mk" data-field="x_Sesi" data-value-separator="<?php echo $master_mk->Sesi->DisplayValueSeparatorAttribute() ?>" id="x<?php echo $master_mk_list->RowIndex ?>_Sesi" name="x<?php echo $master_mk_list->RowIndex ?>_Sesi"<?php echo $master_mk->Sesi->EditAttributes() ?>>
...
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Sesi" class="master_mk_Sesi">
...
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Wajib" class="form-group master_mk_Wajib">
<div id="tp_x<?php echo $master_mk_list->RowIndex ?>_Wajib" class="ewTemplate"><input type="radio" data-table="master_mk" data-field="x_Wajib" data-value-separator="<?php echo $master_mk->Wajib->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $master_mk_list->RowIndex ?>_Wajib" id="x<?php echo $master_mk_list->RowIndex ?>_Wajib" value="{value}"<?php echo $master_mk->Wajib->EditAttributes() ?>></div>
<div id="dsl_x<?php echo $master_mk_list->RowIndex ?>_Wajib" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $master_mk->Wajib->RadioButtonListHtml(FALSE, "x{$master_mk_list->RowIndex}_Wajib") ?>
</div></div>
</span>
<input type="hidden" data-table="master_mk" data-field="x_Wajib" name="o<?php echo $master_mk_list->RowIndex ?>_Wajib" id="o<?php echo $master_mk_list->RowIndex ?>_Wajib" value="<?php echo ew_HtmlEncode($master_mk->Wajib->OldValue) ?>">
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Wajib" class="form-group master_mk_Wajib">
<div id="tp_x<?php echo $master_mk_list->RowIndex ?>_Wajib" class="ewTemplate"><input type="radio" data-table="master_mk" data-field="x_Wajib" data-value-separator="<?php echo $master_mk->Wajib->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $master_mk_list->RowIndex ?>_Wajib" id="x<?php echo $master_mk_list->RowIndex ?>_Wajib" value="{value}"<?php echo $master_mk->Wajib->EditAttributes() ?>></div>
...
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_Wajib" class="master_mk_Wajib">
...
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD) { // Add record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_NA" class="form-group master_mk_NA">
<div id="tp_x<?php echo $master_mk_list->RowIndex ?>_NA" class="ewTemplate"><input type="radio" data-table="master_mk" data-field="x_NA" data-value-separator="<?php echo $master_mk->NA->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $master_mk_list->RowIndex ?>_NA" id="x<?php echo $master_mk_list->RowIndex ?>_NA" value="{value}"<?php echo $master_mk->NA->EditAttributes() ?>></div>
<div id="dsl_x<?php echo $master_mk_list->RowIndex ?>_NA" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $master_mk->NA->RadioButtonListHtml(FALSE, "x{$master_mk_list->RowIndex}_NA") ?>
</div></div>
</span>
<input type="hidden" data-table="master_mk" data-field="x_NA" name="o<?php echo $master_mk_list->RowIndex ?>_NA" id="o<?php echo $master_mk_list->RowIndex ?>_NA" value="<?php echo ew_HtmlEncode($master_mk->NA->OldValue) ?>">
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_EDIT) { // Edit record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_NA" class="form-group master_mk_NA">
<div id="tp_x<?php echo $master_mk_list->RowIndex ?>_NA" class="ewTemplate"><input type="radio" data-table="master_mk" data-field="x_NA" data-value-separator="<?php echo $master_mk->NA->DisplayValueSeparatorAttribute() ?>" name="x<?php echo $master_mk_list->RowIndex ?>_NA" id="x<?php echo $master_mk_list->RowIndex ?>_NA" value="{value}"<?php echo $master_mk->NA->EditAttributes() ?>></div>
...
<?php } ?>
<?php if ($master_mk->RowType == EW_ROWTYPE_VIEW) { // View record ?>
<span id="el<?php echo $master_mk_list->RowCnt ?>_master_mk_NA" class="master_mk_NA">
...
$master_mk_list->ListOptions->Render("body", "right", $master_mk_list->RowCnt);
?>
	</tr>
<?php if ($master_mk->RowType == EW_ROWTYPE_ADD || $master_mk->RowType == EW_ROWTYPE_EDIT) { ?>
<script type="text/javascript">
fmaster_mklist.UpdateOpts(<?php echo $master_mk_list->RowIndex ?>);
</script>
<?php } ?>
<?php
	}
	...
	if ($master_mk->CurrentAction <> "gridadd")
		if (!$master_mk_list->Recordset->EOF) $master_mk_list->Recordset->MoveNext();
}
?>
<?php
	if ($master_mk->CurrentAction == "gridadd" || $master_mk->CurrentAction == "gridedit") {
		$master_mk_list->RowIndex = '$rowindex$';
		$master_mk_list->LoadDefaultValues();

		// Set row properties
		$master_mk->ResetAttrs();
		$master_mk->RowAttrs = array_merge($master_mk->RowAttrs, array('data-rowindex'=>$master_mk_list->RowIndex, 'id'=>'r0_master_mk', 'data-rowtype'=>EW_ROWTYPE_ADD));
		...
		// Render list options
		$master_mk_list->RenderListOptions();
		$master_mk_list->StartRowCnt = 0;
?>
	<tr<?php echo $master_mk->RowAttributes() ?>>
<?php

// Render list options (body, left)
$master_mk_list->ListOptions->Render("body", "left", $master_mk_list->RowIndex);
?>
	<?php if ($master_mk->MKKode->Visible) { // MKKode ?>
		<td data-name="MKKode">
...
<?php } ?>
<?php if ($master_mk->CurrentAction == "gridadd") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridinsert">
<input type="hidden" name="<?php echo $master_mk_list->FormKeyCountName ?>" id="<?php echo $master_mk_list->FormKeyCountName ?>" value="<?php echo $master_mk_list->KeyCount ?>">
<?php echo $master_mk_list->MultiSelectKey ?>
<?php } ?>
<?php if ($master_mk->CurrentAction == "gridedit") { ?>
<input type="hidden" name="a_list" id="a_list" value="gridupdate">
<input type="hidden" name="<?php echo $master_mk_list->FormKeyCountName ?>" id="<?php echo $master_mk_list->FormKeyCountName ?>" value="<?php echo $master_mk_list->KeyCount ?>">
<?php echo $master_mk_list->MultiSelectKey ?>
<?php } ?>
<?php if ($master_mk->CurrentAction == "") { ?>
...
// Close recordset
if ($master_mk_list->Recordset)
	$master_mk_list->Recordset->Close();
?>
<?php if ($master_mk->Export == "") { ?>
...
<?php } ?>
<?php if ($master_mk_list->TotalRecs == 0 && $master_mk->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($master_mk_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($master_mk->Export == "") { ?>
<script type="text/javascript">
fmaster_mklistsrch.FilterList = <?php echo $master_mk_list->GetFilterList() ?>;
fmaster_mklistsrch.Init();
fmaster_mklist.Init();
</script>
<?php } ?>
<?php
$master_mk_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($master_mk->Export == "") { ?>
