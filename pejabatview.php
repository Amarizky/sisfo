...
$pejabat_view = NULL; // Initialize page object first

class cpejabat_view extends cpejabat {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["JabatanID"] <> "") {
			$this->RecKey["JabatanID"] = $_GET["JabatanID"];
			$KeyUrl .= "&amp;JabatanID=" . urlencode($this->RecKey["JabatanID"]);
		}
		if (@$_GET["KodeID"] <> "") {
			$this->RecKey["KodeID"] = $_GET["KodeID"];
			$KeyUrl .= "&amp;KodeID=" . urlencode($this->RecKey["KodeID"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["JabatanID"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["JabatanID"]);
		}
		if (@$_GET["KodeID"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["KodeID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["JabatanID"] <> "") {
				...
				$this->RecKey["JabatanID"] = $this->JabatanID->QueryStringValue;
			} elseif (@$_POST["JabatanID"] <> "") {
				$this->JabatanID->setFormValue($_POST["JabatanID"]);
				$this->RecKey["JabatanID"] = $this->JabatanID->FormValue;
			} else {
				$sReturnUrl = "pejabatlist.php"; // Return to list
			}
			if (@$_GET["KodeID"] <> "") {
				$this->KodeID->setQueryStringValue($_GET["KodeID"]);
				$this->RecKey["KodeID"] = $this->KodeID->QueryStringValue;
			} elseif (@$_POST["KodeID"] <> "") {
				$this->KodeID->setFormValue($_POST["KodeID"]);
				$this->RecKey["KodeID"] = $this->KodeID->FormValue;
			} else {
				$sReturnUrl = "pejabatlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "pejabatlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "pejabatlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($pejabat_view)) $pejabat_view = new cpejabat_view();

// Page init
$pejabat_view->Page_Init();

// Page main
$pejabat_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pejabat_view->Page_Render();
?>
...
var CurrentForm = fpejabatview = new ew_Form("fpejabatview", "view");

// Form_CustomValidate event
fpejabatview.Form_CustomValidate = 
...
fpejabatview.ValidateRequired = true;
<?php } else { ?>
fpejabatview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpejabatview.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpejabatview.Lists["x_NA"].Options = <?php echo json_encode($pejabat->NA->Options()) ?>;
...
<?php if ($pejabat->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$pejabat_view->IsModal) { ?>
<?php if ($pejabat->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $pejabat_view->ExportOptions->Render("body") ?>
<?php
	foreach ($pejabat_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$pejabat_view->IsModal) { ?>
<?php if ($pejabat->Export == "") { ?>
...
<?php } ?>
<?php $pejabat_view->ShowPageHeader(); ?>
<?php
$pejabat_view->ShowMessage();
?>
<form name="fpejabatview" id="fpejabatview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pejabat_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pejabat_view->Token ?>">
<?php } ?>
...
<?php echo $pejabat->JabatanID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $pejabat->Nama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $pejabat->Report->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $pejabat->NA->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $pejabat->KodeID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $pejabat->LevelID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $pejabat->Ranking->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
