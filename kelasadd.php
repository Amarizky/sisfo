...
$kelas_add = NULL; // Initialize page object first

class ckelas_add extends ckelas {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["KelasID"] != "") {
				...
				$this->setKey("KelasID", $this->KelasID->CurrentValue); // Set up key
			} else {
				$this->setKey("KelasID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "kelasview.php")
...
// Create page object
if (!isset($kelas_add)) $kelas_add = new ckelas_add();
...
var CurrentForm = fkelasadd = new ew_Form("fkelasadd", "add");

// Validate form
fkelasadd.Validate = function() {
...
fkelasadd.ValidateRequired = true;
<?php } else { ?>
fkelasadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkelasadd.Lists["x_ProdiID"] = {"LinkField":"x_ProdiID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_prodi"};
fkelasadd.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkelasadd.Lists["x_NA"].Options = <?php echo json_encode($kelas->NA->Options()) ?>;
...
<?php } ?>
<?php $kelas_add->ShowPageHeader(); ?>
<?php
$kelas_add->ShowMessage();
?>
<form name="fkelasadd" id="fkelasadd" class="<?php echo $kelas_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kelas_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kelas_add->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$kelas_add->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $kelas_add->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_kelasadd" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kelas_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
