...
$kurikulum_delete = NULL; // Initialize page object first

class ckurikulum_delete extends ckurikulum {
	...
	// Page object name
	var $PageObjName = 'kurikulum_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("kurikulumlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in kurikulum class, kurikuluminfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("kurikulumlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($kurikulum_delete)) $kurikulum_delete = new ckurikulum_delete();

// Page init
$kurikulum_delete->Page_Init();

// Page main
$kurikulum_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kurikulum_delete->Page_Render();
?>
...
var CurrentForm = fkurikulumdelete = new ew_Form("fkurikulumdelete", "delete");

// Form_CustomValidate event
fkurikulumdelete.Form_CustomValidate = 
...
fkurikulumdelete.ValidateRequired = true;
<?php } else { ?>
fkurikulumdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkurikulumdelete.Lists["x_ProdiID"] = {"LinkField":"x_ProdiID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_prodi"};
fkurikulumdelete.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkurikulumdelete.Lists["x_NA"].Options = <?php echo json_encode($kurikulum->NA->Options()) ?>;
...
<?php $kurikulum_delete->ShowPageHeader(); ?>
<?php
$kurikulum_delete->ShowMessage();
?>
<form name="fkurikulumdelete" id="fkurikulumdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kurikulum_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kurikulum_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="kurikulum">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($kurikulum_delete->RecKeys as $key) { ?>
...
<?php if ($kurikulum->ProdiID->Visible) { // ProdiID ?>
		<th><span id="elh_kurikulum_ProdiID" class="kurikulum_ProdiID"><?php echo $kurikulum->ProdiID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($kurikulum->KurikulumKode->Visible) { // KurikulumKode ?>
		<th><span id="elh_kurikulum_KurikulumKode" class="kurikulum_KurikulumKode"><?php echo $kurikulum->KurikulumKode->FldCaption() ?></span></th>
<?php } ?>
<?php if ($kurikulum->Nama->Visible) { // Nama ?>
		<th><span id="elh_kurikulum_Nama" class="kurikulum_Nama"><?php echo $kurikulum->Nama->FldCaption() ?></span></th>
<?php } ?>
<?php if ($kurikulum->JmlSesi->Visible) { // JmlSesi ?>
		<th><span id="elh_kurikulum_JmlSesi" class="kurikulum_JmlSesi"><?php echo $kurikulum->JmlSesi->FldCaption() ?></span></th>
<?php } ?>
<?php if ($kurikulum->NA->Visible) { // NA ?>
		<th><span id="elh_kurikulum_NA" class="kurikulum_NA"><?php echo $kurikulum->NA->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$kurikulum_delete->RecCnt = 0;
$i = 0;
while (!$kurikulum_delete->Recordset->EOF) {
	$kurikulum_delete->RecCnt++;
	$kurikulum_delete->RowCnt++;

	// Set row properties
	$kurikulum->ResetAttrs();
	$kurikulum->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$kurikulum_delete->LoadRowValues($kurikulum_delete->Recordset);

	// Render row
	$kurikulum_delete->RenderRow();
?>
	<tr<?php echo $kurikulum->RowAttributes() ?>>
<?php if ($kurikulum->ProdiID->Visible) { // ProdiID ?>
		<td<?php echo $kurikulum->ProdiID->CellAttributes() ?>>
<span id="el<?php echo $kurikulum_delete->RowCnt ?>_kurikulum_ProdiID" class="kurikulum_ProdiID">
...
<span id="el<?php echo $kurikulum_delete->RowCnt ?>_kurikulum_KurikulumKode" class="kurikulum_KurikulumKode">
...
<span id="el<?php echo $kurikulum_delete->RowCnt ?>_kurikulum_Nama" class="kurikulum_Nama">
...
<span id="el<?php echo $kurikulum_delete->RowCnt ?>_kurikulum_JmlSesi" class="kurikulum_JmlSesi">
...
<span id="el<?php echo $kurikulum_delete->RowCnt ?>_kurikulum_NA" class="kurikulum_NA">
...
<?php
	$kurikulum_delete->Recordset->MoveNext();
}
$kurikulum_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kurikulum_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fkurikulumdelete.Init();
</script>
<?php
$kurikulum_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kurikulum_delete->Page_Terminate();
?>
