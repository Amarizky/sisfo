...
//
// Table class for master_jamkul
//
class cmaster_jamkul extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'master_jamkul';
		$this->TableName = 'master_jamkul';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`master_jamkul`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// HariID
		$this->HariID = new cField('master_jamkul', 'master_jamkul', 'x_HariID', 'HariID', '`HariID`', '`HariID`', 3, -1, FALSE, '`HariID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// JamID
		$this->JamID = new cField('master_jamkul', 'master_jamkul', 'x_JamID', 'JamID', '`JamID`', '`JamID`', 200, -1, FALSE, '`JamID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->JamID->Sortable = TRUE; // Allow sort
		$this->fields['JamID'] = &$this->JamID;

		// JamMulai
		$this->JamMulai = new cField('master_jamkul', 'master_jamkul', 'x_JamMulai', 'JamMulai', '`JamMulai`', ew_CastDateFieldForLike('`JamMulai`', 0, "DB"), 134, -1, FALSE, '`JamMulai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// JamSelesai
		$this->JamSelesai = new cField('master_jamkul', 'master_jamkul', 'x_JamSelesai', 'JamSelesai', '`JamSelesai`', ew_CastDateFieldForLike('`JamSelesai`', 0, "DB"), 134, -1, FALSE, '`JamSelesai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// NA
		$this->NA = new cField('master_jamkul', 'master_jamkul', 'x_NA', 'NA', '`NA`', '`NA`', 202, -1, FALSE, '`NA`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`master_jamkul`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('HariID', $rs))
				ew_AddFilter($where, ew_QuotedName('HariID', $this->DBID) . '=' . ew_QuotedValue($rs['HariID'], $this->HariID->FldDataType, $this->DBID));
			if (array_key_exists('JamID', $rs))
				ew_AddFilter($where, ew_QuotedName('JamID', $this->DBID) . '=' . ew_QuotedValue($rs['JamID'], $this->JamID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`HariID` = @HariID@ AND `JamID` = '@JamID@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->HariID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@HariID@", ew_AdjustSql($this->HariID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		$sKeyFilter = str_replace("@JamID@", ew_AdjustSql($this->JamID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("master_jamkulview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("master_jamkulview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "master_jamkuladd.php?" . $this->UrlParm($parm);
		else
			$url = "master_jamkuladd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("master_jamkuledit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("master_jamkuladd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("master_jamkuldelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "HariID:" . ew_VarToJson($this->HariID->CurrentValue, "number", "'");
		$json .= ",JamID:" . ew_VarToJson($this->JamID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->HariID->CurrentValue)) {
			$sUrl .= "HariID=" . urlencode($this->HariID->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		if (!is_null($this->JamID->CurrentValue)) {
			$sUrl .= "&JamID=" . urlencode($this->JamID->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["HariID"]))
				$arKey[] = ew_StripSlashes($_POST["HariID"]);
			elseif (isset($_GET["HariID"]))
				$arKey[] = ew_StripSlashes($_GET["HariID"]);
			else
				$arKeys = NULL; // Do not setup
			if ($isPost && isset($_POST["JamID"]))
				$arKey[] = ew_StripSlashes($_POST["JamID"]);
			elseif (isset($_GET["JamID"]))
				$arKey[] = ew_StripSlashes($_GET["JamID"]);
			else
				$arKeys = NULL; // Do not setup
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->HariID->CurrentValue = $key[0];
			$this->JamID->CurrentValue = $key[1];
