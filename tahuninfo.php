...
//
// Table class for tahun
//
class ctahun extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'tahun';
		$this->TableName = 'tahun';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`tahun`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// TahunID
		$this->TahunID = new cField('tahun', 'tahun', 'x_TahunID', 'TahunID', '`TahunID`', '`TahunID`', 200, -1, FALSE, '`TahunID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->TahunID->Sortable = TRUE; // Allow sort
		$this->fields['TahunID'] = &$this->TahunID;

		// Sesi
		$this->Sesi = new cField('tahun', 'tahun', 'x_Sesi', 'Sesi', '`Sesi`', '`Sesi`', 3, -1, FALSE, '`Sesi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// ProdiID
		$this->ProdiID = new cField('tahun', 'tahun', 'x_ProdiID', 'ProdiID', '`ProdiID`', '`ProdiID`', 200, -1, FALSE, '`ProdiID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Nama
		$this->Nama = new cField('tahun', 'tahun', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Nama->Sortable = TRUE; // Allow sort
		$this->fields['Nama'] = &$this->Nama;

		// Creator
		$this->Creator = new cField('tahun', 'tahun', 'x_Creator', 'Creator', '`Creator`', '`Creator`', 200, -1, FALSE, '`Creator`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Creator->Sortable = TRUE; // Allow sort
		$this->fields['Creator'] = &$this->Creator;

		// CreateDate
		$this->CreateDate = new cField('tahun', 'tahun', 'x_CreateDate', 'CreateDate', '`CreateDate`', ew_CastDateFieldForLike('`CreateDate`', 0, "DB"), 135, 0, FALSE, '`CreateDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// EditDate
		$this->EditDate = new cField('tahun', 'tahun', 'x_EditDate', 'EditDate', '`EditDate`', ew_CastDateFieldForLike('`EditDate`', 0, "DB"), 135, 0, FALSE, '`EditDate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Editor
		$this->Editor = new cField('tahun', 'tahun', 'x_Editor', 'Editor', '`Editor`', '`Editor`', 200, -1, FALSE, '`Editor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Editor->Sortable = TRUE; // Allow sort
		$this->fields['Editor'] = &$this->Editor;

		// Catatan
		$this->Catatan = new cField('tahun', 'tahun', 'x_Catatan', 'Catatan', '`Catatan`', '`Catatan`', 201, -1, FALSE, '`Catatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->Catatan->Sortable = TRUE; // Allow sort
		$this->fields['Catatan'] = &$this->Catatan;

		// NA
		$this->NA = new cField('tahun', 'tahun', 'x_NA', 'NA', '`NA`', '`NA`', 202, -1, FALSE, '`NA`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`tahun`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('TahunID', $rs))
				ew_AddFilter($where, ew_QuotedName('TahunID', $this->DBID) . '=' . ew_QuotedValue($rs['TahunID'], $this->TahunID->FldDataType, $this->DBID));
			if (array_key_exists('Sesi', $rs))
				ew_AddFilter($where, ew_QuotedName('Sesi', $this->DBID) . '=' . ew_QuotedValue($rs['Sesi'], $this->Sesi->FldDataType, $this->DBID));
			if (array_key_exists('ProdiID', $rs))
				ew_AddFilter($where, ew_QuotedName('ProdiID', $this->DBID) . '=' . ew_QuotedValue($rs['ProdiID'], $this->ProdiID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`TahunID` = '@TahunID@' AND `Sesi` = @Sesi@ AND `ProdiID` = '@ProdiID@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@TahunID@", ew_AdjustSql($this->TahunID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		if (!is_numeric($this->Sesi->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@Sesi@", ew_AdjustSql($this->Sesi->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		$sKeyFilter = str_replace("@ProdiID@", ew_AdjustSql($this->ProdiID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("tahunview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("tahunview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "tahunadd.php?" . $this->UrlParm($parm);
		else
			$url = "tahunadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("tahunedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("tahunadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("tahundelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "TahunID:" . ew_VarToJson($this->TahunID->CurrentValue, "string", "'");
		$json .= ",Sesi:" . ew_VarToJson($this->Sesi->CurrentValue, "number", "'");
		$json .= ",ProdiID:" . ew_VarToJson($this->ProdiID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->TahunID->CurrentValue)) {
			$sUrl .= "TahunID=" . urlencode($this->TahunID->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		if (!is_null($this->Sesi->CurrentValue)) {
			$sUrl .= "&Sesi=" . urlencode($this->Sesi->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		if (!is_null($this->ProdiID->CurrentValue)) {
			$sUrl .= "&ProdiID=" . urlencode($this->ProdiID->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["TahunID"]))
				$arKey[] = ew_StripSlashes($_POST["TahunID"]);
			elseif (isset($_GET["TahunID"]))
				$arKey[] = ew_StripSlashes($_GET["TahunID"]);
			else
				$arKeys = NULL; // Do not setup
			if ($isPost && isset($_POST["Sesi"]))
				$arKey[] = ew_StripSlashes($_POST["Sesi"]);
			elseif (isset($_GET["Sesi"]))
				$arKey[] = ew_StripSlashes($_GET["Sesi"]);
			else
				$arKeys = NULL; // Do not setup
			if ($isPost && isset($_POST["ProdiID"]))
				$arKey[] = ew_StripSlashes($_POST["ProdiID"]);
			elseif (isset($_GET["ProdiID"]))
				$arKey[] = ew_StripSlashes($_GET["ProdiID"]);
			else
				$arKeys = NULL; // Do not setup
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->TahunID->CurrentValue = $key[0];
			$this->Sesi->CurrentValue = $key[1];
			$this->ProdiID->CurrentValue = $key[2];
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$arwrk = array();
				$arwrk[1] = $rswrk->fields('DispFld');
				$this->Sesi->EditValue = $this->Sesi->DisplayValue($arwrk);
				$rswrk->Close();
			} else {
				$this->Sesi->EditValue = $this->Sesi->CurrentValue;
			}
		} else {
			$this->Sesi->EditValue = NULL;
		}
		$this->Sesi->ViewCustomAttributes = "";
		...
		if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			...
			if ($rswrk && !$rswrk->EOF) { // Lookup values found
				$arwrk = array();
				$arwrk[1] = $rswrk->fields('DispFld');
				$this->ProdiID->EditValue = $this->ProdiID->DisplayValue($arwrk);
				$rswrk->Close();
			} else {
				$this->ProdiID->EditValue = $this->ProdiID->CurrentValue;
			}
		} else {
			$this->ProdiID->EditValue = NULL;
		}
		$this->ProdiID->ViewCustomAttributes = "";
	...
	function GetAutoFill($id, $val) {
		$rsarr = array();
		$rowcnt = 0;
		if (preg_match('/^x(\d)*_ProdiID$/', $id)) {
			$conn = &$this->Connection();
			$sSqlWrk = "SELECT `Nama` AS FIELD0 FROM `master_prodi`";
			$sWhereWrk = "(`ProdiID` = " . ew_QuotedValue($val, EW_DATATYPE_STRING, $this->DBID) . ")";
			$this->ProdiID->LookupFilters = array();
			$this->Lookup_Selecting($this->ProdiID, $sWhereWrk); // Call Lookup selecting
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
			if ($rs = ew_LoadRecordset($sSqlWrk, $conn)) {
				while ($rs && !$rs->EOF) {
					$ar = array();
					$this->Nama->setDbValue($rs->fields[0]);
					$this->RowType == EW_ROWTYPE_EDIT;
					$this->RenderEditRow();
					$ar[] = ($this->Nama->AutoFillOriginalValue) ? $this->Nama->CurrentValue : $this->Nama->EditValue;
					$rowcnt += 1;
					$rsarr[] = $ar;
					$rs->MoveNext();
				}
