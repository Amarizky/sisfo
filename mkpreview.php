...
$mk_preview = NULL; // Initialize page object first

class cmk_preview extends cmk {
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'mk');
...
// Create page object
if (!isset($mk_preview)) $mk_preview = new cmk_preview();

// Page init
$mk_preview->Page_Init();

// Page main
$mk_preview->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$mk_preview->Page_Render();
?>
<?php $mk_preview->ShowPageHeader(); ?>
<?php if ($mk_preview->TotalRecs > 0) { ?>
<div class="ewGrid">
...
// Render list options
$mk_preview->RenderListOptions();

// Render list options (header, left)
$mk_preview->ListOptions->Render("header", "left");
?>
<?php if ($mk->MKKode->Visible) { // MKKode ?>
			<th><?php echo $mk->MKKode->FldCaption() ?></th>
<?php } ?>
<?php if ($mk->Nama->Visible) { // Nama ?>
			<th><?php echo $mk->Nama->FldCaption() ?></th>
<?php } ?>
<?php if ($mk->Singkatan->Visible) { // Singkatan ?>
			<th><?php echo $mk->Singkatan->FldCaption() ?></th>
<?php } ?>
<?php if ($mk->Tingkat->Visible) { // Tingkat ?>
			<th><?php echo $mk->Tingkat->FldCaption() ?></th>
<?php } ?>
<?php if ($mk->Sesi->Visible) { // Sesi ?>
			<th><?php echo $mk->Sesi->FldCaption() ?></th>
<?php } ?>
<?php if ($mk->Wajib->Visible) { // Wajib ?>
			<th><?php echo $mk->Wajib->FldCaption() ?></th>
<?php } ?>
<?php if ($mk->NA->Visible) { // NA ?>
			<th><?php echo $mk->NA->FldCaption() ?></th>
<?php } ?>
<?php

// Render list options (header, right)
$mk_preview->ListOptions->Render("header", "right");
?>
		</tr>
	</thead>
	<tbody><!-- Table body -->
<?php
$mk_preview->RecCount = 0;
$mk_preview->RowCnt = 0;
while ($mk_preview->Recordset && !$mk_preview->Recordset->EOF) {

	// Init row class and style
	$mk_preview->RecCount++;
	$mk_preview->RowCnt++;
	$mk_preview->CssStyle = "";
	$mk_preview->LoadListRowValues($mk_preview->Recordset);

	// Render row
	$mk_preview->RowType = EW_ROWTYPE_PREVIEW; // Preview record
	$mk_preview->ResetAttrs();
	$mk_preview->RenderListRow();

	// Render list options
	$mk_preview->RenderListOptions();
?>
	<tr<?php echo $mk_preview->RowAttributes() ?>>
<?php

// Render list options (body, left)
$mk_preview->ListOptions->Render("body", "left", $mk_preview->RowCnt);
?>
<?php if ($mk->MKKode->Visible) { // MKKode ?>
		<!-- MKKode -->
...
$mk_preview->ListOptions->Render("body", "right", $mk_preview->RowCnt);
?>
	</tr>
<?php
	$mk_preview->Recordset->MoveNext();
}
?>
	</tbody>
</table>
...
<?php if ($mk_preview->TotalRecs > 0) { ?>
<div class="ewDetailCount"><?php echo $mk_preview->TotalRecs ?>&nbsp;<?php echo $Language->Phrase("Record") ?></div>
<?php } else { ?>
<div class="ewDetailCount"><?php echo $Language->Phrase("NoRecord") ?></div>
<?php } ?>
<div class="ewPreviewOtherOptions">
<?php
	foreach ($mk_preview->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
...
<?php
$mk_preview->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php
if ($mk_preview->Recordset)
	$mk_preview->Recordset->Close();
