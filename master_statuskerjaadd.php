...
$master_statuskerja_add = NULL; // Initialize page object first

class cmaster_statuskerja_add extends cmaster_statuskerja {
	...
	// Page object name
	var $PageObjName = 'master_statuskerja_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["StatusKerjaID"] != "") {
				...
				$this->setKey("StatusKerjaID", $this->StatusKerjaID->CurrentValue); // Set up key
			} else {
				$this->setKey("StatusKerjaID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "master_statuskerjaview.php")
	...
	function RenderRow() {
		...
		$this->NA->ViewCustomAttributes = "";
			...
			$this->StatusKerjaID->EditValue = ew_HtmlEncode($this->StatusKerjaID->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		// StatusKerjaID
		$this->StatusKerjaID->SetDbValueDef($rsnew, $this->StatusKerjaID->CurrentValue, "", FALSE);
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['StatusKerjaID']) == "") {
...
// Create page object
if (!isset($master_statuskerja_add)) $master_statuskerja_add = new cmaster_statuskerja_add();

// Page init
$master_statuskerja_add->Page_Init();

// Page main
$master_statuskerja_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_statuskerja_add->Page_Render();
?>
...
var CurrentForm = fmaster_statuskerjaadd = new ew_Form("fmaster_statuskerjaadd", "add");

// Validate form
fmaster_statuskerjaadd.Validate = function() {
...
fmaster_statuskerjaadd.Form_CustomValidate = 
...
fmaster_statuskerjaadd.ValidateRequired = true;
<?php } else { ?>
fmaster_statuskerjaadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmaster_statuskerjaadd.Lists["x_Def"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_statuskerjaadd.Lists["x_Def"].Options = <?php echo json_encode($master_statuskerja->Def->Options()) ?>;
fmaster_statuskerjaadd.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_statuskerjaadd.Lists["x_NA"].Options = <?php echo json_encode($master_statuskerja->NA->Options()) ?>;
...
<?php if (!$master_statuskerja_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $master_statuskerja_add->ShowPageHeader(); ?>
<?php
$master_statuskerja_add->ShowMessage();
?>
<form name="fmaster_statuskerjaadd" id="fmaster_statuskerjaadd" class="<?php echo $master_statuskerja_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_statuskerja_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_statuskerja_add->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$master_statuskerja_add->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $master_statuskerja_add->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_master_statuskerjaadd" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $master_statuskerja_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fmaster_statuskerjaadd.Init();
</script>
<?php
$master_statuskerja_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$master_statuskerja_add->Page_Terminate();
?>
