...
$mk_delete = NULL; // Initialize page object first

class cmk_delete extends cmk {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("mklist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in mk class, mkinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("mklist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($mk_delete)) $mk_delete = new cmk_delete();
...
var CurrentForm = fmkdelete = new ew_Form("fmkdelete", "delete");

// Form_CustomValidate event
fmkdelete.Form_CustomValidate = 
...
fmkdelete.ValidateRequired = true;
<?php } else { ?>
fmkdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmkdelete.Lists["x_Tingkat[]"] = {"LinkField":"x_Tingkat","Ajax":true,"AutoFill":false,"DisplayFields":["x_Tingkat","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"kelas"};
fmkdelete.Lists["x_Sesi"] = {"LinkField":"x_Sesi","Ajax":true,"AutoFill":false,"DisplayFields":["x_NamaSesi","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_sesi"};
fmkdelete.Lists["x_Wajib"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmkdelete.Lists["x_Wajib"].Options = <?php echo json_encode($mk->Wajib->Options()) ?>;
fmkdelete.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmkdelete.Lists["x_NA"].Options = <?php echo json_encode($mk->NA->Options()) ?>;
...
<?php $mk_delete->ShowPageHeader(); ?>
<?php
$mk_delete->ShowMessage();
?>
<form name="fmkdelete" id="fmkdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($mk_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $mk_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="mk">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($mk_delete->RecKeys as $key) { ?>
...
<?php if ($mk->MKKode->Visible) { // MKKode ?>
		<th><span id="elh_mk_MKKode" class="mk_MKKode"><?php echo $mk->MKKode->FldCaption() ?></span></th>
<?php } ?>
<?php if ($mk->Nama->Visible) { // Nama ?>
		<th><span id="elh_mk_Nama" class="mk_Nama"><?php echo $mk->Nama->FldCaption() ?></span></th>
<?php } ?>
<?php if ($mk->Singkatan->Visible) { // Singkatan ?>
		<th><span id="elh_mk_Singkatan" class="mk_Singkatan"><?php echo $mk->Singkatan->FldCaption() ?></span></th>
<?php } ?>
<?php if ($mk->Tingkat->Visible) { // Tingkat ?>
		<th><span id="elh_mk_Tingkat" class="mk_Tingkat"><?php echo $mk->Tingkat->FldCaption() ?></span></th>
<?php } ?>
<?php if ($mk->Sesi->Visible) { // Sesi ?>
		<th><span id="elh_mk_Sesi" class="mk_Sesi"><?php echo $mk->Sesi->FldCaption() ?></span></th>
<?php } ?>
<?php if ($mk->Wajib->Visible) { // Wajib ?>
		<th><span id="elh_mk_Wajib" class="mk_Wajib"><?php echo $mk->Wajib->FldCaption() ?></span></th>
<?php } ?>
<?php if ($mk->NA->Visible) { // NA ?>
		<th><span id="elh_mk_NA" class="mk_NA"><?php echo $mk->NA->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
$mk_delete->RecCnt = 0;
$i = 0;
while (!$mk_delete->Recordset->EOF) {
	$mk_delete->RecCnt++;
	$mk_delete->RowCnt++;

	// Set row properties
	$mk->ResetAttrs();
	$mk->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$mk_delete->LoadRowValues($mk_delete->Recordset);
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_MKKode" class="mk_MKKode">
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_Nama" class="mk_Nama">
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_Singkatan" class="mk_Singkatan">
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_Tingkat" class="mk_Tingkat">
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_Sesi" class="mk_Sesi">
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_Wajib" class="mk_Wajib">
...
<span id="el<?php echo $mk_delete->RowCnt ?>_mk_NA" class="mk_NA">
...
<?php
	$mk_delete->Recordset->MoveNext();
}
$mk_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $mk_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmkdelete.Init();
