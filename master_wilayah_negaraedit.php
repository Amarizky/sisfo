...
$master_wilayah_negara_edit = NULL; // Initialize page object first

class cmaster_wilayah_negara_edit extends cmaster_wilayah_negara {
	...
	// Page object name
	var $PageObjName = 'master_wilayah_negara_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->NegaraID->CurrentValue == "") {
			$this->Page_Terminate("master_wilayah_negaralist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->BenuaID->SetDbValueDef($rsnew, $this->BenuaID->CurrentValue, "", $this->BenuaID->ReadOnly);

			// NamaBenua
			$this->NamaBenua->SetDbValueDef($rsnew, $this->NamaBenua->CurrentValue, "", $this->NamaBenua->ReadOnly);

			// KodeNegara
			$this->KodeNegara->SetDbValueDef($rsnew, $this->KodeNegara->CurrentValue, "", $this->KodeNegara->ReadOnly);

			// NamaNegara
			$this->NamaNegara->SetDbValueDef($rsnew, $this->NamaNegara->CurrentValue, "", $this->NamaNegara->ReadOnly);
...
// Create page object
if (!isset($master_wilayah_negara_edit)) $master_wilayah_negara_edit = new cmaster_wilayah_negara_edit();

// Page init
$master_wilayah_negara_edit->Page_Init();

// Page main
$master_wilayah_negara_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_wilayah_negara_edit->Page_Render();
?>
...
var CurrentForm = fmaster_wilayah_negaraedit = new ew_Form("fmaster_wilayah_negaraedit", "edit");

// Validate form
fmaster_wilayah_negaraedit.Validate = function() {
...
fmaster_wilayah_negaraedit.Form_CustomValidate = 
...
fmaster_wilayah_negaraedit.ValidateRequired = true;
<?php } else { ?>
fmaster_wilayah_negaraedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmaster_wilayah_negaraedit.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_wilayah_negaraedit.Lists["x_NA"].Options = <?php echo json_encode($master_wilayah_negara->NA->Options()) ?>;
...
<?php if (!$master_wilayah_negara_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $master_wilayah_negara_edit->ShowPageHeader(); ?>
<?php
$master_wilayah_negara_edit->ShowMessage();
?>
<form name="fmaster_wilayah_negaraedit" id="fmaster_wilayah_negaraedit" class="<?php echo $master_wilayah_negara_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_wilayah_negara_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_wilayah_negara_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$master_wilayah_negara_edit->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $master_wilayah_negara_edit->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_master_wilayah_negaraedit" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
<?php if ($master_wilayah_negara->NegaraID->Visible) { // NegaraID ?>
<?php if (ew_IsMobile() || $master_wilayah_negara_edit->IsModal) { ?>
	<div id="r_NegaraID" class="form-group">
		<label id="elh_master_wilayah_negara_NegaraID" class="col-sm-2 control-label ewLabel"><?php echo $master_wilayah_negara->NegaraID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $master_wilayah_negara->NegaraID->CellAttributes() ?>>
<span id="el_master_wilayah_negara_NegaraID">
<span<?php echo $master_wilayah_negara->NegaraID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $master_wilayah_negara->NegaraID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="master_wilayah_negara" data-field="x_NegaraID" name="x_NegaraID" id="x_NegaraID" value="<?php echo ew_HtmlEncode($master_wilayah_negara->NegaraID->CurrentValue) ?>">
<?php echo $master_wilayah_negara->NegaraID->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_NegaraID">
...
<input type="hidden" data-table="master_wilayah_negara" data-field="x_NegaraID" name="x_NegaraID" id="x_NegaraID" value="<?php echo ew_HtmlEncode($master_wilayah_negara->NegaraID->CurrentValue) ?>">
<?php echo $master_wilayah_negara->NegaraID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $master_wilayah_negara_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fmaster_wilayah_negaraedit.Init();
</script>
<?php
$master_wilayah_negara_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$master_wilayah_negara_edit->Page_Terminate();
?>
