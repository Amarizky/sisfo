...
$master_wilayah_desa = NULL;

//
// Table class for master_wilayah_desa
//
class cmaster_wilayah_desa extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'master_wilayah_desa';
		$this->TableName = 'master_wilayah_desa';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`master_wilayah_desa`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// DesaID
		$this->DesaID = new cField('master_wilayah_desa', 'master_wilayah_desa', 'x_DesaID', 'DesaID', '`DesaID`', '`DesaID`', 200, -1, FALSE, '`DesaID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->DesaID->Sortable = TRUE; // Allow sort
		$this->fields['DesaID'] = &$this->DesaID;

		// KecamatanID
		$this->KecamatanID = new cField('master_wilayah_desa', 'master_wilayah_desa', 'x_KecamatanID', 'KecamatanID', '`KecamatanID`', '`KecamatanID`', 200, -1, FALSE, '`KecamatanID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// Desa
		$this->Desa = new cField('master_wilayah_desa', 'master_wilayah_desa', 'x_Desa', 'Desa', '`Desa`', '`Desa`', 200, -1, FALSE, '`Desa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`master_wilayah_desa`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('DesaID', $rs))
				ew_AddFilter($where, ew_QuotedName('DesaID', $this->DBID) . '=' . ew_QuotedValue($rs['DesaID'], $this->DesaID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`DesaID` = '@DesaID@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@DesaID@", ew_AdjustSql($this->DesaID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "master_wilayah_desalist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("master_wilayah_desaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("master_wilayah_desaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "master_wilayah_desaadd.php?" . $this->UrlParm($parm);
		else
			$url = "master_wilayah_desaadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("master_wilayah_desaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("master_wilayah_desaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("master_wilayah_desadelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "DesaID:" . ew_VarToJson($this->DesaID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->DesaID->CurrentValue)) {
			$sUrl .= "DesaID=" . urlencode($this->DesaID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["DesaID"]))
				$arKeys[] = ew_StripSlashes($_POST["DesaID"]);
			elseif (isset($_GET["DesaID"]))
				$arKeys[] = ew_StripSlashes($_GET["DesaID"]);
			else
