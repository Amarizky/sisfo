...
$krs_edit = NULL; // Initialize page object first

class ckrs_edit extends ckrs {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->KRSID->CurrentValue == "") {
			$this->Page_Terminate("krslist.php"); // Invalid key, return to list
		}
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->KRSID->FldIsDetailKey)
			$this->KRSID->setFormValue($objForm->GetValue("x_KRSID"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->KRSID->CurrentValue = $this->KRSID->FormValue;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// KHSID
			$this->KHSID->SetDbValueDef($rsnew, $this->KHSID->CurrentValue, 0, $this->KHSID->ReadOnly);

			// StudentID
			$this->StudentID->SetDbValueDef($rsnew, $this->StudentID->CurrentValue, "", $this->StudentID->ReadOnly);
			...
			// JadwalID
			$this->JadwalID->SetDbValueDef($rsnew, $this->JadwalID->CurrentValue, NULL, $this->JadwalID->ReadOnly);

			// MKID
			$this->MKID->SetDbValueDef($rsnew, $this->MKID->CurrentValue, NULL, $this->MKID->ReadOnly);

			// MKKode
			$this->MKKode->SetDbValueDef($rsnew, $this->MKKode->CurrentValue, NULL, $this->MKKode->ReadOnly);

			// SKS
			$this->SKS->SetDbValueDef($rsnew, $this->SKS->CurrentValue, NULL, $this->SKS->ReadOnly);

			// Tugas1
			$this->Tugas1->SetDbValueDef($rsnew, $this->Tugas1->CurrentValue, NULL, $this->Tugas1->ReadOnly);

			// Tugas2
			$this->Tugas2->SetDbValueDef($rsnew, $this->Tugas2->CurrentValue, NULL, $this->Tugas2->ReadOnly);

			// Tugas3
			$this->Tugas3->SetDbValueDef($rsnew, $this->Tugas3->CurrentValue, NULL, $this->Tugas3->ReadOnly);

			// Tugas4
			$this->Tugas4->SetDbValueDef($rsnew, $this->Tugas4->CurrentValue, NULL, $this->Tugas4->ReadOnly);

			// Tugas5
			$this->Tugas5->SetDbValueDef($rsnew, $this->Tugas5->CurrentValue, NULL, $this->Tugas5->ReadOnly);

			// Presensi
			$this->Presensi->SetDbValueDef($rsnew, $this->Presensi->CurrentValue, NULL, $this->Presensi->ReadOnly);

			// _Presensi
			$this->_Presensi->SetDbValueDef($rsnew, $this->_Presensi->CurrentValue, NULL, $this->_Presensi->ReadOnly);

			// UTS
			$this->UTS->SetDbValueDef($rsnew, $this->UTS->CurrentValue, NULL, $this->UTS->ReadOnly);

			// UAS
			$this->UAS->SetDbValueDef($rsnew, $this->UAS->CurrentValue, NULL, $this->UAS->ReadOnly);

			// Responsi
			$this->Responsi->SetDbValueDef($rsnew, $this->Responsi->CurrentValue, NULL, $this->Responsi->ReadOnly);

			// NilaiAkhir
			$this->NilaiAkhir->SetDbValueDef($rsnew, $this->NilaiAkhir->CurrentValue, NULL, $this->NilaiAkhir->ReadOnly);

			// GradeNilai
			$this->GradeNilai->SetDbValueDef($rsnew, $this->GradeNilai->CurrentValue, NULL, $this->GradeNilai->ReadOnly);

			// BobotNilai
			$this->BobotNilai->SetDbValueDef($rsnew, $this->BobotNilai->CurrentValue, NULL, $this->BobotNilai->ReadOnly);

			// StatusKRSID
			$this->StatusKRSID->SetDbValueDef($rsnew, $this->StatusKRSID->CurrentValue, "", $this->StatusKRSID->ReadOnly);

			// Tinggi
			$this->Tinggi->SetDbValueDef($rsnew, $this->Tinggi->CurrentValue, NULL, $this->Tinggi->ReadOnly);

			// Final
			$this->Final->SetDbValueDef($rsnew, ((strval($this->Final->CurrentValue) == "Y") ? "Y" : "N"), NULL, $this->Final->ReadOnly);

			// Setara
			$this->Setara->SetDbValueDef($rsnew, ((strval($this->Setara->CurrentValue) == "Y") ? "Y" : "N"), NULL, $this->Setara->ReadOnly);

			// Creator
			$this->Creator->SetDbValueDef($rsnew, $this->Creator->CurrentValue, NULL, $this->Creator->ReadOnly);

			// CreateDate
			$this->CreateDate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->CreateDate->CurrentValue, 0), NULL, $this->CreateDate->ReadOnly);

			// Editor
			$this->Editor->SetDbValueDef($rsnew, $this->Editor->CurrentValue, NULL, $this->Editor->ReadOnly);

			// EditDate
			$this->EditDate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->EditDate->CurrentValue, 0), NULL, $this->EditDate->ReadOnly);
...
// Create page object
if (!isset($krs_edit)) $krs_edit = new ckrs_edit();
...
var CurrentForm = fkrsedit = new ew_Form("fkrsedit", "edit");

// Validate form
fkrsedit.Validate = function() {
...
fkrsedit.ValidateRequired = true;
<?php } else { ?>
fkrsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkrsedit.Lists["x_Final"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkrsedit.Lists["x_Final"].Options = <?php echo json_encode($krs->Final->Options()) ?>;
fkrsedit.Lists["x_Setara"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkrsedit.Lists["x_Setara"].Options = <?php echo json_encode($krs->Setara->Options()) ?>;
fkrsedit.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkrsedit.Lists["x_NA"].Options = <?php echo json_encode($krs->NA->Options()) ?>;
...
<?php } ?>
<?php $krs_edit->ShowPageHeader(); ?>
<?php
$krs_edit->ShowMessage();
?>
<form name="fkrsedit" id="fkrsedit" class="<?php echo $krs_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($krs_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $krs_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$krs_edit->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $krs_edit->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_krsedit" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
<?php if ($krs->KRSID->Visible) { // KRSID ?>
<?php if (ew_IsMobile() || $krs_edit->IsModal) { ?>
	<div id="r_KRSID" class="form-group">
		<label id="elh_krs_KRSID" class="col-sm-2 control-label ewLabel"><?php echo $krs->KRSID->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $krs->KRSID->CellAttributes() ?>>
<span id="el_krs_KRSID">
<span<?php echo $krs->KRSID->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $krs->KRSID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="krs" data-field="x_KRSID" name="x_KRSID" id="x_KRSID" value="<?php echo ew_HtmlEncode($krs->KRSID->CurrentValue) ?>">
<?php echo $krs->KRSID->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_KRSID">
...
<input type="hidden" data-table="krs" data-field="x_KRSID" name="x_KRSID" id="x_KRSID" value="<?php echo ew_HtmlEncode($krs->KRSID->CurrentValue) ?>">
<?php echo $krs->KRSID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $krs_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
