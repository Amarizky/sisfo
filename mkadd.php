...
$mk_add = NULL; // Initialize page object first

class cmk_add extends cmk {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["MKID"] != "") {
				...
				$this->setKey("MKID", $this->MKID->CurrentValue); // Set up key
			} else {
				$this->setKey("MKID", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "mkview.php")
	...
	function LoadDefaultValues() {
		...
		$this->KurikulumID->CurrentValue = 0;
	...
	function AddRow($rsold = NULL) {
		...
		if (strval($this->KurikulumID->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@KurikulumID@", ew_AdjustSql($this->KurikulumID->CurrentValue, "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if (strval($this->ProdiID->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@ProdiID@", ew_AdjustSql($this->ProdiID->CurrentValue, "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if (strval($this->KampusID->CurrentValue) <> "") {
			$sMasterFilter = str_replace("@KampusID@", ew_AdjustSql($this->KampusID->CurrentValue, "DB"), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		...
		// KurikulumID
		$this->KurikulumID->SetDbValueDef($rsnew, $this->KurikulumID->CurrentValue, 0, strval($this->KurikulumID->CurrentValue) == "");
...
// Create page object
if (!isset($mk_add)) $mk_add = new cmk_add();
...
var CurrentForm = fmkadd = new ew_Form("fmkadd", "add");

// Validate form
fmkadd.Validate = function() {
...
fmkadd.ValidateRequired = true;
<?php } else { ?>
fmkadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmkadd.Lists["x_KampusID"] = {"LinkField":"x_KampusID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_kampus"};
fmkadd.Lists["x_ProdiID"] = {"LinkField":"x_ProdiID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":["x_KurikulumID"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_prodi"};
fmkadd.Lists["x_KurikulumID"] = {"LinkField":"x_KurikulumID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":["x_ProdiID"],"ChildFields":[],"FilterFields":["x_ProdiID"],"Options":[],"Template":"","LinkTable":"kurikulum"};
fmkadd.Lists["x_Tingkat[]"] = {"LinkField":"x_Tingkat","Ajax":true,"AutoFill":false,"DisplayFields":["x_Tingkat","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"kelas"};
fmkadd.Lists["x_Sesi"] = {"LinkField":"x_Sesi","Ajax":true,"AutoFill":false,"DisplayFields":["x_NamaSesi","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_sesi"};
fmkadd.Lists["x_Wajib"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmkadd.Lists["x_Wajib"].Options = <?php echo json_encode($mk->Wajib->Options()) ?>;
...
<?php } ?>
<?php $mk_add->ShowPageHeader(); ?>
<?php
$mk_add->ShowMessage();
?>
<form name="fmkadd" id="fmkadd" class="<?php echo $mk_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($mk_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $mk_add->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$mk_add->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $mk_add->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_mkadd" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $mk_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
