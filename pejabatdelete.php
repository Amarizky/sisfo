...
$pejabat_delete = NULL; // Initialize page object first

class cpejabat_delete extends cpejabat {
	...
	// Page object name
	var $PageObjName = 'pejabat_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("pejabatlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in pejabat class, pejabatinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("pejabatlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($pejabat_delete)) $pejabat_delete = new cpejabat_delete();

// Page init
$pejabat_delete->Page_Init();

// Page main
$pejabat_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pejabat_delete->Page_Render();
?>
...
var CurrentForm = fpejabatdelete = new ew_Form("fpejabatdelete", "delete");

// Form_CustomValidate event
fpejabatdelete.Form_CustomValidate = 
...
fpejabatdelete.ValidateRequired = true;
<?php } else { ?>
fpejabatdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpejabatdelete.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fpejabatdelete.Lists["x_NA"].Options = <?php echo json_encode($pejabat->NA->Options()) ?>;
...
<?php $pejabat_delete->ShowPageHeader(); ?>
<?php
$pejabat_delete->ShowMessage();
?>
<form name="fpejabatdelete" id="fpejabatdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pejabat_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pejabat_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pejabat">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($pejabat_delete->RecKeys as $key) { ?>
...
<?php if ($pejabat->JabatanID->Visible) { // JabatanID ?>
		<th><span id="elh_pejabat_JabatanID" class="pejabat_JabatanID"><?php echo $pejabat->JabatanID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pejabat->Nama->Visible) { // Nama ?>
		<th><span id="elh_pejabat_Nama" class="pejabat_Nama"><?php echo $pejabat->Nama->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pejabat->Report->Visible) { // Report ?>
		<th><span id="elh_pejabat_Report" class="pejabat_Report"><?php echo $pejabat->Report->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pejabat->NA->Visible) { // NA ?>
		<th><span id="elh_pejabat_NA" class="pejabat_NA"><?php echo $pejabat->NA->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pejabat->KodeID->Visible) { // KodeID ?>
		<th><span id="elh_pejabat_KodeID" class="pejabat_KodeID"><?php echo $pejabat->KodeID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pejabat->LevelID->Visible) { // LevelID ?>
		<th><span id="elh_pejabat_LevelID" class="pejabat_LevelID"><?php echo $pejabat->LevelID->FldCaption() ?></span></th>
<?php } ?>
<?php if ($pejabat->Ranking->Visible) { // Ranking ?>
		<th><span id="elh_pejabat_Ranking" class="pejabat_Ranking"><?php echo $pejabat->Ranking->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$pejabat_delete->RecCnt = 0;
$i = 0;
while (!$pejabat_delete->Recordset->EOF) {
	$pejabat_delete->RecCnt++;
	$pejabat_delete->RowCnt++;

	// Set row properties
	$pejabat->ResetAttrs();
	$pejabat->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$pejabat_delete->LoadRowValues($pejabat_delete->Recordset);

	// Render row
	$pejabat_delete->RenderRow();
?>
	<tr<?php echo $pejabat->RowAttributes() ?>>
<?php if ($pejabat->JabatanID->Visible) { // JabatanID ?>
		<td<?php echo $pejabat->JabatanID->CellAttributes() ?>>
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_JabatanID" class="pejabat_JabatanID">
...
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_Nama" class="pejabat_Nama">
...
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_Report" class="pejabat_Report">
...
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_NA" class="pejabat_NA">
...
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_KodeID" class="pejabat_KodeID">
...
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_LevelID" class="pejabat_LevelID">
...
<span id="el<?php echo $pejabat_delete->RowCnt ?>_pejabat_Ranking" class="pejabat_Ranking">
...
<?php
	$pejabat_delete->Recordset->MoveNext();
}
$pejabat_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pejabat_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fpejabatdelete.Init();
