...
$master_jenjang_edit = NULL; // Initialize page object first

class cmaster_jenjang_edit extends cmaster_jenjang {
	...
	// Page object name
	var $PageObjName = 'master_jenjang_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->JenjangID->CurrentValue == "") {
			$this->Page_Terminate("master_jenjanglist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Nama_en
			$this->Nama_en->SetDbValueDef($rsnew, $this->Nama_en->CurrentValue, "", $this->Nama_en->ReadOnly);
...
// Create page object
if (!isset($master_jenjang_edit)) $master_jenjang_edit = new cmaster_jenjang_edit();

// Page init
$master_jenjang_edit->Page_Init();

// Page main
$master_jenjang_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_jenjang_edit->Page_Render();
?>
...
var CurrentForm = fmaster_jenjangedit = new ew_Form("fmaster_jenjangedit", "edit");

// Validate form
fmaster_jenjangedit.Validate = function() {
...
fmaster_jenjangedit.Form_CustomValidate = 
...
fmaster_jenjangedit.ValidateRequired = true;
<?php } else { ?>
fmaster_jenjangedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmaster_jenjangedit.Lists["x_Def"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_jenjangedit.Lists["x_Def"].Options = <?php echo json_encode($master_jenjang->Def->Options()) ?>;
fmaster_jenjangedit.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_jenjangedit.Lists["x_NA"].Options = <?php echo json_encode($master_jenjang->NA->Options()) ?>;
...
<?php if (!$master_jenjang_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $master_jenjang_edit->ShowPageHeader(); ?>
<?php
$master_jenjang_edit->ShowMessage();
?>
<form name="fmaster_jenjangedit" id="fmaster_jenjangedit" class="<?php echo $master_jenjang_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_jenjang_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_jenjang_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$master_jenjang_edit->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $master_jenjang_edit->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_master_jenjangedit" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
...
<p class="form-control-static"><?php echo $master_jenjang->JenjangID->EditValue ?></p></span>
</span>
<input type="hidden" data-table="master_jenjang" data-field="x_JenjangID" name="x_JenjangID" id="x_JenjangID" value="<?php echo ew_HtmlEncode($master_jenjang->JenjangID->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $master_jenjang_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fmaster_jenjangedit.Init();
</script>
<?php
$master_jenjang_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$master_jenjang_edit->Page_Terminate();
?>
