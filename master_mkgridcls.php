...
$master_mk_grid = NULL; // Initialize page object first

class cmaster_mk_grid extends cmaster_mk {
	...
	// Page object name
	var $PageObjName = 'master_mk_grid';

	// Grid form hidden field names
	var $FormName = 'fmaster_mkgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["master_mk"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->MKID->CurrentValue = strval($arKeys[0]); // MKID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "kurikulum") {
			...
			if ($GLOBALS["kurikulum"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->ProdiID->Visible = FALSE;
			if ($GLOBALS["kurikulum"]->EventCancelled) $this->EventCancelled = TRUE;
			$this->KampusID->Visible = FALSE;
			if ($GLOBALS["kurikulum"]->EventCancelled) $this->EventCancelled = TRUE;
		}
