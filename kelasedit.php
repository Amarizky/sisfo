...
$kelas_edit = NULL; // Initialize page object first

class ckelas_edit extends ckelas {
	...
	function Page_Main() {
		...
		if (@$_GET["KelasID"] <> "") {
			$this->KelasID->setQueryStringValue($_GET["KelasID"]);
		}
		...
		// Check if valid key
		if ($this->KelasID->CurrentValue == "") {
			$this->Page_Terminate("kelaslist.php"); // Invalid key, return to list
		}
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("kelaslist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
				$sReturnUrl = $this->getReturnUrl();
				if (ew_GetPageName($sReturnUrl) == "kelaslist.php")
...
// Create page object
if (!isset($kelas_edit)) $kelas_edit = new ckelas_edit();

// Page init
$kelas_edit->Page_Init();

// Page main
$kelas_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kelas_edit->Page_Render();
?>
...
var CurrentForm = fkelasedit = new ew_Form("fkelasedit", "edit");

// Validate form
fkelasedit.Validate = function() {
...
fkelasedit.Form_CustomValidate = 
...
fkelasedit.ValidateRequired = true;
<?php } else { ?>
fkelasedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkelasedit.Lists["x_ProdiID"] = {"LinkField":"x_ProdiID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_prodi"};
fkelasedit.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkelasedit.Lists["x_NA"].Options = <?php echo json_encode($kelas->NA->Options()) ?>;
...
<?php } ?>
<?php $kelas_edit->ShowPageHeader(); ?>
<?php
$kelas_edit->ShowMessage();
?>
<form name="fkelasedit" id="fkelasedit" class="<?php echo $kelas_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kelas_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kelas_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<?php if (!ew_IsMobile() && !$kelas_edit->IsModal) { ?>
<div class="ewDesktop">
<?php } ?>
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
<div>
<?php } else { ?>
<div>
<table id="tbl_kelasedit" class="table table-bordered table-striped ewDesktopTable">
<?php } ?>
<?php if ($kelas->KelasID->Visible) { // KelasID ?>
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
	<div id="r_KelasID" class="form-group">
		<label id="elh_kelas_KelasID" for="x_KelasID" class="col-sm-2 control-label ewLabel"><?php echo $kelas->KelasID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $kelas->KelasID->CellAttributes() ?>>
...
<input type="hidden" data-table="kelas" data-field="x_KelasID" name="x_KelasID" id="x_KelasID" value="<?php echo ew_HtmlEncode($kelas->KelasID->CurrentValue) ?>">
<?php echo $kelas->KelasID->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_KelasID">
		<td><span id="elh_kelas_KelasID"><?php echo $kelas->KelasID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<input type="hidden" data-table="kelas" data-field="x_KelasID" name="x_KelasID" id="x_KelasID" value="<?php echo ew_HtmlEncode($kelas->KelasID->CurrentValue) ?>">
<?php echo $kelas->KelasID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
	<div id="r_Tingkat" class="form-group">
		<label id="elh_kelas_Tingkat" for="x_Tingkat" class="col-sm-2 control-label ewLabel"><?php echo $kelas->Tingkat->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kelas->Tingkat->CellAttributes() ?>>
...
<?php echo $kelas->Tingkat->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_Tingkat">
...
<?php echo $kelas->Tingkat->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
	<div id="r_Nama" class="form-group">
		<label id="elh_kelas_Nama" for="x_Nama" class="col-sm-2 control-label ewLabel"><?php echo $kelas->Nama->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kelas->Nama->CellAttributes() ?>>
...
<?php echo $kelas->Nama->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_Nama">
...
<?php echo $kelas->Nama->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
	<div id="r_ProdiID" class="form-group">
		<label id="elh_kelas_ProdiID" for="x_ProdiID" class="col-sm-2 control-label ewLabel"><?php echo $kelas->ProdiID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $kelas->ProdiID->CellAttributes() ?>>
...
<?php echo $kelas->ProdiID->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_ProdiID">
		<td><span id="elh_kelas_ProdiID"><?php echo $kelas->ProdiID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php echo $kelas->ProdiID->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
...
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
	<div id="r_NA" class="form-group">
		<label id="elh_kelas_NA" class="col-sm-2 control-label ewLabel"><?php echo $kelas->NA->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $kelas->NA->CellAttributes() ?>>
<span id="el_kelas_NA">
...
<?php echo $kelas->NA->CustomMsg ?></div></div>
	</div>
<?php } else { ?>
	<tr id="r_NA">
		<td><span id="elh_kelas_NA"><?php echo $kelas->NA->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
...
<?php echo $kelas->NA->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php } ?>
<?php if (ew_IsMobile() || $kelas_edit->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kelas_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</div>
<?php } ?>
</form>
