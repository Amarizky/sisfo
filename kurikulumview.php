...
$kurikulum_view = NULL; // Initialize page object first

class ckurikulum_view extends ckurikulum {
	...
	// Page object name
	var $PageObjName = 'kurikulum_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["KurikulumID"] <> "") {
			$this->RecKey["KurikulumID"] = $_GET["KurikulumID"];
			$KeyUrl .= "&amp;KurikulumID=" . urlencode($this->RecKey["KurikulumID"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["KurikulumID"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["KurikulumID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["KurikulumID"] <> "") {
				...
				$this->RecKey["KurikulumID"] = $this->KurikulumID->QueryStringValue;
			} elseif (@$_POST["KurikulumID"] <> "") {
				$this->KurikulumID->setFormValue($_POST["KurikulumID"]);
				$this->RecKey["KurikulumID"] = $this->KurikulumID->FormValue;
			} else {
				$sReturnUrl = "kurikulumlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "kurikulumlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "kurikulumlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_mk"
		$item = &$option->Add("detail_mk");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("mk", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["mk_grid"] && $GLOBALS["mk_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'mk')) {
		...
		if ($GLOBALS["mk_grid"] && $GLOBALS["mk_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'mk')) {
		...
		if ($GLOBALS["mk_grid"] && $GLOBALS["mk_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'mk')) {
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'mk');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (mk)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("mk", explode(",", $this->getCurrentDetailTable()))) {
			global $mk;
			if (!isset($mk)) $mk = new cmk;
			$rsdetail = $mk->LoadRs($mk->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$mk->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("mk", $DetailTblVar)) {
				...
				if ($GLOBALS["mk_grid"]->DetailView) {
					$GLOBALS["mk_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($kurikulum_view)) $kurikulum_view = new ckurikulum_view();

// Page init
$kurikulum_view->Page_Init();

// Page main
$kurikulum_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kurikulum_view->Page_Render();
?>
...
var CurrentForm = fkurikulumview = new ew_Form("fkurikulumview", "view");

// Form_CustomValidate event
fkurikulumview.Form_CustomValidate = 
...
fkurikulumview.ValidateRequired = true;
<?php } else { ?>
fkurikulumview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fkurikulumview.Lists["x_KampusID"] = {"LinkField":"x_KampusID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_kampus"};
fkurikulumview.Lists["x_ProdiID"] = {"LinkField":"x_ProdiID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_prodi"};
fkurikulumview.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fkurikulumview.Lists["x_NA"].Options = <?php echo json_encode($kurikulum->NA->Options()) ?>;
...
<?php if ($kurikulum->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$kurikulum_view->IsModal) { ?>
<?php if ($kurikulum->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $kurikulum_view->ExportOptions->Render("body") ?>
<?php
	foreach ($kurikulum_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$kurikulum_view->IsModal) { ?>
<?php if ($kurikulum->Export == "") { ?>
...
<?php } ?>
<?php $kurikulum_view->ShowPageHeader(); ?>
<?php
$kurikulum_view->ShowMessage();
?>
<form name="fkurikulumview" id="fkurikulumview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kurikulum_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kurikulum_view->Token ?>">
<?php } ?>
...
<?php if ($kurikulum->KurikulumID->Visible) { // KurikulumID ?>
	<tr id="r_KurikulumID">
		...
		<td data-name="KurikulumID"<?php echo $kurikulum->KurikulumID->CellAttributes() ?>>
<span id="el_kurikulum_KurikulumID">
<span<?php echo $kurikulum->KurikulumID->ViewAttributes() ?>>
<?php echo $kurikulum->KurikulumID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->KampusID->Visible) { // KampusID ?>
	<tr id="r_KampusID">
		<td><span id="elh_kurikulum_KampusID"><?php echo $kurikulum->KampusID->FldCaption() ?></span></td>
		<td data-name="KampusID"<?php echo $kurikulum->KampusID->CellAttributes() ?>>
<span id="el_kurikulum_KampusID">
<span<?php echo $kurikulum->KampusID->ViewAttributes() ?>>
<?php echo $kurikulum->KampusID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->ProdiID->Visible) { // ProdiID ?>
	<tr id="r_ProdiID">
		<td><span id="elh_kurikulum_ProdiID"><?php echo $kurikulum->ProdiID->FldCaption() ?></span></td>
...
<?php echo $kurikulum->ProdiID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->KurikulumKode->Visible) { // KurikulumKode ?>
	<tr id="r_KurikulumKode">
		<td><span id="elh_kurikulum_KurikulumKode"><?php echo $kurikulum->KurikulumKode->FldCaption() ?></span></td>
...
<?php echo $kurikulum->KurikulumKode->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->Nama->Visible) { // Nama ?>
	<tr id="r_Nama">
		<td><span id="elh_kurikulum_Nama"><?php echo $kurikulum->Nama->FldCaption() ?></span></td>
...
<?php echo $kurikulum->Nama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->JmlSesi->Visible) { // JmlSesi ?>
	<tr id="r_JmlSesi">
		<td><span id="elh_kurikulum_JmlSesi"><?php echo $kurikulum->JmlSesi->FldCaption() ?></span></td>
...
<?php echo $kurikulum->JmlSesi->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->CreateDate->Visible) { // CreateDate ?>
	<tr id="r_CreateDate">
		<td><span id="elh_kurikulum_CreateDate"><?php echo $kurikulum->CreateDate->FldCaption() ?></span></td>
		<td data-name="CreateDate"<?php echo $kurikulum->CreateDate->CellAttributes() ?>>
<span id="el_kurikulum_CreateDate">
<span<?php echo $kurikulum->CreateDate->ViewAttributes() ?>>
<?php echo $kurikulum->CreateDate->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->Creator->Visible) { // Creator ?>
	<tr id="r_Creator">
		<td><span id="elh_kurikulum_Creator"><?php echo $kurikulum->Creator->FldCaption() ?></span></td>
		<td data-name="Creator"<?php echo $kurikulum->Creator->CellAttributes() ?>>
<span id="el_kurikulum_Creator">
<span<?php echo $kurikulum->Creator->ViewAttributes() ?>>
<?php echo $kurikulum->Creator->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->EditDate->Visible) { // EditDate ?>
	<tr id="r_EditDate">
		<td><span id="elh_kurikulum_EditDate"><?php echo $kurikulum->EditDate->FldCaption() ?></span></td>
		<td data-name="EditDate"<?php echo $kurikulum->EditDate->CellAttributes() ?>>
<span id="el_kurikulum_EditDate">
<span<?php echo $kurikulum->EditDate->ViewAttributes() ?>>
<?php echo $kurikulum->EditDate->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($kurikulum->Editor->Visible) { // Editor ?>
	<tr id="r_Editor">
		<td><span id="elh_kurikulum_Editor"><?php echo $kurikulum->Editor->FldCaption() ?></span></td>
		<td data-name="Editor"<?php echo $kurikulum->Editor->CellAttributes() ?>>
<span id="el_kurikulum_Editor">
<span<?php echo $kurikulum->Editor->ViewAttributes() ?>>
<?php echo $kurikulum->Editor->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("mk", explode(",", $kurikulum->getCurrentDetailTable())) && $mk->DetailView) {
?>
