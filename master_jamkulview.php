...
$master_jamkul_view = NULL; // Initialize page object first

class cmaster_jamkul_view extends cmaster_jamkul {
	...
	// Page object name
	var $PageObjName = 'master_jamkul_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["HariID"] <> "") {
			$this->RecKey["HariID"] = $_GET["HariID"];
			$KeyUrl .= "&amp;HariID=" . urlencode($this->RecKey["HariID"]);
		}
		if (@$_GET["JamID"] <> "") {
			$this->RecKey["JamID"] = $_GET["JamID"];
			$KeyUrl .= "&amp;JamID=" . urlencode($this->RecKey["JamID"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["HariID"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["HariID"]);
		}
		if (@$_GET["JamID"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["JamID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["HariID"] <> "") {
				...
				$this->RecKey["HariID"] = $this->HariID->QueryStringValue;
			} elseif (@$_POST["HariID"] <> "") {
				$this->HariID->setFormValue($_POST["HariID"]);
				$this->RecKey["HariID"] = $this->HariID->FormValue;
			} else {
				$sReturnUrl = "master_jamkullist.php"; // Return to list
			}
			if (@$_GET["JamID"] <> "") {
				$this->JamID->setQueryStringValue($_GET["JamID"]);
				$this->RecKey["JamID"] = $this->JamID->QueryStringValue;
			} elseif (@$_POST["JamID"] <> "") {
				$this->JamID->setFormValue($_POST["JamID"]);
				$this->RecKey["JamID"] = $this->JamID->FormValue;
			} else {
				$sReturnUrl = "master_jamkullist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "master_jamkullist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "master_jamkullist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($master_jamkul_view)) $master_jamkul_view = new cmaster_jamkul_view();

// Page init
$master_jamkul_view->Page_Init();

// Page main
$master_jamkul_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$master_jamkul_view->Page_Render();
?>
...
var CurrentForm = fmaster_jamkulview = new ew_Form("fmaster_jamkulview", "view");

// Form_CustomValidate event
fmaster_jamkulview.Form_CustomValidate = 
...
fmaster_jamkulview.ValidateRequired = true;
<?php } else { ?>
fmaster_jamkulview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmaster_jamkulview.Lists["x_HariID"] = {"LinkField":"x_HariID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"master_hari"};
fmaster_jamkulview.Lists["x_NA"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmaster_jamkulview.Lists["x_NA"].Options = <?php echo json_encode($master_jamkul->NA->Options()) ?>;
...
<?php if ($master_jamkul->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$master_jamkul_view->IsModal) { ?>
<?php if ($master_jamkul->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $master_jamkul_view->ExportOptions->Render("body") ?>
<?php
	foreach ($master_jamkul_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$master_jamkul_view->IsModal) { ?>
<?php if ($master_jamkul->Export == "") { ?>
...
<?php } ?>
<?php $master_jamkul_view->ShowPageHeader(); ?>
<?php
$master_jamkul_view->ShowMessage();
?>
<form name="fmaster_jamkulview" id="fmaster_jamkulview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($master_jamkul_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $master_jamkul_view->Token ?>">
<?php } ?>
...
<?php echo $master_jamkul->HariID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $master_jamkul->JamID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $master_jamkul->JamMulai->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $master_jamkul->JamSelesai->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $master_jamkul->NA->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($master_jamkul->Export == "") { ?>
<script type="text/javascript">
fmaster_jamkulview.Init();
</script>
<?php } ?>
<?php
$master_jamkul_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($master_jamkul->Export == "") { ?>
...
<?php
$master_jamkul_view->Page_Terminate();
?>
